-- The teacher rostered on a drop-in confirms the hours they actually worked and
-- saves their own payroll shift. SECURITY DEFINER so the rate is derived here
-- (from the year-band matrix) rather than trusted from the client, and so the
-- caller can only ever write a shift for themselves, on a session they are on.
...
begin
  ...
  if v_hours > 12 then
    raise exception 'A drop-in shift cannot be longer than 12 hours';
  end if;
...
revoke all on function public.save_dropin_shift(uuid, time, time) from public;
grant execute on function public.save_dropin_shift(uuid, time, time) to authenticated;
