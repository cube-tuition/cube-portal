import { richToHtml, inlineRich } from './_qblib.mjs'
const qbankImageUrl = (p) => p
const nodeToJpeg = async () => ''
...
function questionBlock(q, index, { includeMarks, answers, writingSpace = true }) {
  const num = index + 1
  ...
  let body = `<div style="font-size:16px;line-height:1.55;color:${INK}">${richToHtml(q.stem_latex || '')}</div>`
  ...
  if (isMcq) {
    ...
    for (const opt of q.options) {
      ...
      body += `<div style="font-size:16px;line-height:1.6;color:${INK};${isCorrect ? 'font-weight:600' : ''}"><span style="font-weight:400;margin-right:4px">${esc(opt.label)}.</span>${inlineRich(opt.latex || '')}${isCorrect ? ` <span style="color:${TICK_GREEN};font-weight:700">✓</span>` : ''}</div>`
    }
    body += '</div>'
  }
...
export async function exportWorksheet({ title, subtitle, questions, includeMarks = true, answers = false, preview = false, renderTo = null, writingSpace = true }) {
  ...
  const blocks = questions.map((q, i) => questionBlock(q, i, { includeMarks, answers, writingSpace }))
  {
...
export { questionBlock, newPage, headerHtml, PAGE_H, PAGE_W }
