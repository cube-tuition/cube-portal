...
function coverPage({ title, cover, answers }) {
  ...
  holder.innerHTML = coverHtml(
    ...
    { solutions: answers, kicker: 'Additional Questions' },
  )
...
export async function exportWorksheet({ title, subtitle, questions, includeMarks = true, answers = false, preview = false, renderTo = null, writingSpace = true, cover = null }) {
  ...
  const blocks = questions.flatMap((q, i) => questionChunks(q, i, { includeMarks, answers, writingSpace }))
  {
  ...
  // open straight onto Question 1 (no grey heading band). The subtitle, when
  // given, rides on the cover's topic line.
  const coverEl = coverPage({ title: subtitle ? `${title || 'Worksheet'} — ${subtitle}` : title, cover, answers })
