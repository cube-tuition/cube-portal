...
// Booklet-style grey heading band: title + CUBE wordmark (+ Answer Key tag),
// with a Name/Date line beneath on the worksheet copy.
function headerHtml(title, subtitle, answers) {
  const wordmark = `<span style="display:flex;flex-direction:column;align-items:flex-end;line-height:1.1">
    <span style="font-weight:600;font-size:15px;letter-spacing:4px;color:${BRAND}">CUBE</span>
    <span style="font-weight:500;font-size:8px;letter-spacing:4px;color:rgba(54,68,102,.55)">TUITION</span>
  </span>`
  const answerTag = answers
    ? `<span style="background:${BRAND};color:#fff;font-weight:600;font-size:10px;letter-spacing:2px;text-transform:uppercase;padding:3px 8px">Answer Key</span>`
    : ''
  return `<div style="margin-bottom:24px">
    <div style="background:${BAND_GREY};padding:22px 26px;display:flex;align-items:center;justify-content:space-between;gap:24px">
      <div style="min-width:0">
        <div style="font-size:26px;font-weight:600;color:${INK};letter-spacing:-.2px">${esc(title) || 'Worksheet'}</div>
        ${subtitle ? `<div style="font-size:14px;color:${MUTED};margin-top:4px">${esc(subtitle)}</div>` : ''}
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;flex:0 0 auto">
        ${wordmark}${answerTag}
      </div>
    </div>
    ${!answers ? `<div style="font-size:14px;color:${INK};margin-top:14px">Name: ______________________________&nbsp;&nbsp;&nbsp;Date: ______________</div>` : ''}
  </div>`
}
