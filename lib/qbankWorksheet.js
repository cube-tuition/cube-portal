...
import { qbankImageUrl, partLabel } from './qbank'
import { nodeToJpeg } from './rasterise'
import { coverHtml, BOOKLET_CSS } from './bookletRender'

// The cover's subject name — "Maths", not "Mathematics", to match how the
// subject is named everywhere else in the portal.
const subjectLong = (v) => {
  if (!v) return 'Maths'
  if (/chem/i.test(v)) return 'Chemistry'
  if (/english|eald/i.test(v)) return 'English'
  if (/math/i.test(v)) return 'Maths'
  return v
}
...
// Booklet-style dotted answer lines (each line LINE_H px, dotted #9a9a9a).
// The group carries its own base count so the page-slack pass (fillPageSlack)
// knows how far it may grow this particular question's working space.
const LINE_H = 26
const lineRowCss = `border-bottom:1px dotted #9a9a9a;height:${LINE_H}px`
function dottedLines(n, fixed = false) {
  const base = Math.max(1, n)
  const rows = Array.from({ length: base })
    .map(() => `<div style="${lineRowCss}"></div>`).join('')
  // `fixed` marks a count the author typed on the worksheet. fillPageSlack
  // leaves those alone — asking for four lines and being handed eight is not
  // a setting, it's a suggestion.
  return `<div class="ws-lines"${fixed ? ' data-fixed="1"' : ''} data-base="${base}" style="margin-top:12px">${rows}</div>`
}
...
// Three lines a mark: a mark in maths is a step of working, and a step that
// has to be squeezed between two dotted lines is a step the marker can't read.
// This is the floor — fillPageSlack() hands back whatever the page has spare
// on top of it.
const LINES_PER_MARK = 3
const linesForMarks = (m, min) => Math.max(min, Math.round((Number(m) || 1) * LINES_PER_MARK))
function linesFor(q, key, marks, min, writingSpace) {
  if (!writingSpace) return { n: 0, fixed: false }
  const override = q?._workingLines?.[key]
  if (override != null && override !== '') return { n: Math.max(0, Number(override) || 0), fixed: true }
  return { n: linesForMarks(marks, min), fixed: false }
}
...
function imagesHtml(images) {
  ...
  const imgs = images.map((im) => {
    ...
    // role 'figure' is a working diagram (a grid to sketch on, a graph to
    // read) — it needs to print large enough to write on, so it gets more
    // room than an illustrative stem image.
    const big = im.role === 'figure'
    return `<img src="${url}" crossorigin="anonymous" alt="${esc(im.alt)}"
      style="max-width:${big ? 440 : 280}px;max-height:${big ? 360 : 200}px;object-fit:contain;margin:6px 8px" />`
  }).join('')
...
function questionChunks(q, index, { includeMarks, answers, writingSpace = true }) {
  ...
  const mk = (html, last, cont = false) => {
    ...
    // Which question this block came from, so the builder can pair the live
    // preview with its tray card (double-click either to jump to the other).
    // Informational only — html-to-image ignores it, so the PDF is unchanged.
    if (q.id != null) el.dataset.qid = String(q.id)
    return el
  }
  ...
  if (parts.length) {
    const partHtml = (p, i) => {
      ...
      if (answers && p.solution_latex?.trim()) answerArea = solBox('Solution:', richToHtml(p.solution_latex))
        ...
        // 0 lines means 0 — an author who zeroes a part, or a sheet built with
        // no writing space at all, should not get one stray rule.
        const { n, fixed } = linesFor(q, lbl, p.marks, 3, writingSpace)
        answerArea = n ? dottedLines(n, fixed) : ''
      }
  ...
  if (answers && q.solution_latex?.trim()) {
    ...
    const { n, fixed } = isMcq ? { n: 0, fixed: false } : linesFor(q, '_', q.marks, 6, writingSpace)
    body += n ? dottedLines(n, fixed) : '<div style="height:8px"></div>'
  }
...
// The page number stands alone — no "3 / 12". The booklet and exam exporters
// have always printed a bare number; this one printed a fraction, so a worksheet
// footer did not match a booklet's sitting beside it.
function footerHtml(pageNo, rightLabel) {
...
/*
 * Hand a page's leftover height back to the students.
 *
 * Pagination packs whole questions, so a page that can't fit the next question
 * ends with a band of blank paper — sometimes a third of the sheet — while the
 * question above it has four cramped lines. Once pagination has settled, the
 * slack on each page is shared out among the working-line groups ON that page,
 * one line at a time, round-robin, so no single question swallows the lot.
 *
 * A line is only kept if the page still fits with it, so this can never push a
 * question off the page or undo the reflow. The cap stops a nearly empty last
 * page turning one short question into a page of ruled lines: a group never
 * more than doubles, and never gains more than MAX_EXTRA lines.
 */
const MAX_EXTRA = 8
function fillPageSlack(pages) {
  for (let p = 1; p < pages.length; p++) {   // 0 is the cover
    const pg = pages[p]
    const groups = Array.from(pg.querySelectorAll('.ws-lines:not([data-fixed])'))
    if (!groups.length) continue
    const added = new Map(groups.map((g) => [g, 0]))
    // A page carrying only one or two groups — a part stranded at the foot of a
    // section, which the next section's band pushes off on its own — has room
    // nobody else can use, so its ceiling lifts.
    const room = groups.length <= 2 ? 2 : 1
    let guard = 0
    let full = false
    while (!full && guard++ < 200) {
      let grew = false
      for (const g of groups) {
        const cap = Math.min(MAX_EXTRA * room, (Number(g.dataset.base) || 1) * room)
        if (added.get(g) >= cap) continue
        const row = document.createElement('div')
        row.style.cssText = lineRowCss
        g.appendChild(row)
        if (pg.scrollHeight > PAGE_H) { g.removeChild(row); full = true; break }
        added.set(g, added.get(g) + 1)
        grew = true
      }
      if (!grew) break
    }
...
// output: 'download' (default) saves the file; 'preview' returns a blob URL for
// the in-page viewer; 'blob' returns the Blob itself, for callers that store
// the PDF (a lesson's assigned worksheet). `preview: true` is the old spelling
// of output: 'preview'.
export async function exportWorksheet({ title, questions, includeMarks = true, answers = false, preview = false, output = null, renderTo = null, writingSpace = true, cover = null, breaks = [] }) {
  ...
  relabel()
  // A question split across a page break reads badly — the student turns the
  // page mid-question — and it leaves the tail stranded on a page of its own
  // at the end of a section. Where the WHOLE question fits on the page its
  // tail landed on, its earlier chunks follow it down; the space they leave
  // behind goes to the questions above them, below. A question too big for one
  // page still splits, as it must.
  const keepTogether = () => {
    for (let i = 2; i < pages.length; i++) {
      const inn = pages[i].firstChild
      const first = inn.firstElementChild
      if (first?.dataset.cont !== '1' || !first.dataset.qid) continue
      const prev = pages[i - 1].firstChild
      const move = []
      for (let n = prev.lastElementChild; n && n.dataset?.qid === first.dataset.qid; n = n.previousElementSibling) move.unshift(n)
      // Nothing to gain if the previous page holds nothing else.
      if (!move.length || move.length === prev.children.length) continue
      move.forEach((n) => inn.insertBefore(n, first))   // each lands just above the tail, so order holds
      // Put them back if the whole question doesn't actually fit here.
      if (pages[i].scrollHeight > PAGE_H) move.forEach((n) => prev.appendChild(n))
    }
  }
  keepTogether()
  relabel()
  // Only now, with every question on its final page, is a page's spare height
  // real — so this is where it can be turned into working space.
  if (writingSpace && !answers) fillPageSlack(pages)
  ...
  pages.forEach((pg, i) => {
    ...
    const holder = document.createElement('div')
    holder.innerHTML = footerHtml(i, footerLabel)
  ...
  const mode = output || (preview ? 'preview' : 'download')
  if (mode === 'blob') return { blob: pdf.output('blob'), filename }
  if (mode === 'preview') return { url: URL.createObjectURL(pdf.output('blob')), filename }
  pdf.save(filename)
}
