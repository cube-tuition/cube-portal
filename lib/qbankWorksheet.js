...
function dottedLines(n) {
  ...
  return `<div class="ws-lines" data-base="${base}" style="margin-top:12px">${rows}</div>`
}
...
function questionChunks(q, index, { includeMarks, answers, writingSpace = true }) {
  ...
  if (parts.length) {
    const partHtml = (p, i) => {
      ...
      if (answers && p.solution_latex?.trim()) answerArea = solBox('Solution:', richToHtml(p.solution_latex))
        ...
        const n = linesFor(q, lbl, p.marks, 3, writingSpace)
        answerArea = n ? dottedLines(n) : ''
      }
  ...
  if (answers && q.solution_latex?.trim()) {
    ...
    const n = isMcq ? 0 : linesFor(q, '_', q.marks, 6, writingSpace)
    body += n ? dottedLines(n) : '<div style="height:8px"></div>'
  }
...
function fillPageSlack(pages) {
  for (let p = 1; p < pages.length; p++) {   // 0 is the cover
    const pg = pages[p]
    const groups = Array.from(pg.querySelectorAll('.ws-lines'))
