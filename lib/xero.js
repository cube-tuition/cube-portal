...
async function xeroFetch(path, { method = 'GET', body } = {}, retries = 3) {
  ...
  if (res.status === 429) {
    // Cap wait at 5s so we don't blow the Vercel timeout.
    // After 3 retries, throw so the caller can surface a clear error.
    if (retries <= 0) throw new Error(`Xero rate limit (429) on ${method} ${path} — please wait a minute and try again`)
    const waitMs = 5000
...
/**
 * Create a new Xero contact (or update existing by name if duplicate).
 * Used when we have no prior xero_contact_id in the DB.
 * Returns the Xero ContactID.
 */
