...
export async function findOrCreateContact({ name, email, phone }) {
  // Search by email
  if (email) {
    const search = await xeroFetch(`/Contacts?where=EmailAddress=="${encodeURIComponent(email)}"`)
    if (search.Contacts?.length > 0) return search.Contacts[0].ContactID
  }

  // Search by name
  const nameSearch = await xeroFetch(`/Contacts?searchTerm=${encodeURIComponent(name)}`)
