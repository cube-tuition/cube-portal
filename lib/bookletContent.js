...
// A "• " line is a main dotpoint; any dash-led line ("— "/"– "/"- ", indented or
// not) is a subdotpoint; anything else is a section header.
export function parseBookletContent(text) {
  ...
  for (const raw of String(text || '').split('\n')) {
    ...
    else if (/^\s*[—–-]\s+/.test(t)) rows.push({ kind: 'sub', text: t.replace(/^\s*[—–-]\s+/, '') })
