 ...
 *
 * What the paper knows varies, so the reading falls back in four steps:
 *   1. A topic typed on the question itself — what a pre-test offers, since an
 *      exam-style paper is question-only and carries no headings.
 *   2. Section/subtopic headings, on papers built as booklets.
 *   3. Questions pulled from the question bank — each carries its bank topic.
 *   4. None of those: one row per question, for the tutor to name and merge.
 *      Marks and question numbers are still filled in, the fiddly part.
 */
...
export function paperQuestions(blocks) {
  ...
  for (const b of blocks || []) {
    ...
    out.push({
      n,
      heading,
      // Typed on the question in the builder. Never printed: it says what the
      // question is for, not what it asks.
      topic: cleanTitle(b.topic),
...
export function topicsFromPaper(blocks, { topicNameByQuestion = {} } = {}) {
  ...
  if (questions.some((q) => q.topic)) {
    return { source: 'tagged', topics: groupBy(questions, (q) => q.topic || 'Other') }
  }
