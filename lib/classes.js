...
/**
 * Every term id a student has an enrolment in, as a Set.
 *
 * Deliberately counts an enrolment that has since ended: this answers "were
 * they ever in a course that term", which is what a history screen is asking.
 */
export async function enrolledTermIdsFor(studentId) {
  if (!studentId) return new Set()
  const { data } = await supabase
    .from(T_ENROLMENTS)
    .select('classes!inner(term_id)')
    .eq('student_id', studentId)
  return new Set((data || []).map(r => r.classes?.term_id).filter(Boolean))
}
