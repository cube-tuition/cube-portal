...
export const DEFAULT_DISCOUNT_CONTENT = {
  ...
  formsNote:        'Adding a course or sibling? New students can [book a free trial](https://www.cubetuition.com.au/free-trial) — for anything else just reply to this email, and we’ll match the details on your invoice automatically.',
...
// [label](url) → a link. Runs after escaping, so label and href are already
// HTML-safe. Only http(s)/mailto targets are linkified — this content is
// staff-editable, so a stray "javascript:" URL is left as plain text.
const linkify = (html, color) => html.replace(
  /\[([^\]]+)\]\((https?:\/\/[^\s)]+|mailto:[^\s)]+)\)/g,
  (_m, label, href) => `<a href="${href}" style="color:${color};font-weight:700;">${label}</a>`,
)

// escape, then **bold** → <strong>, [label](url) → link, newline → <br/>
const rich = (s) => linkify(escHtml(s)
  .replace(/\*\*(.+?)\*\*/g, `<strong style="color:inherit;">$1</strong>`)
  .replace(/\n/g, '<br/>'), BLUE)

// White strong text variant for the dark hero card
const richOnDark = (s) => linkify(escHtml(s)
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
  ...
  <tr><td align="center" style="background:#ffffff;padding:24px 32px 36px;">
    <a href="mailto:${CONTACT_EMAIL}?subject=Referral%20—%20CUBE%20Tuition" style="display:inline-block;background:${BLUE};color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;padding:14px 36px;border-radius:10px;">${rich(c.ctaLabel)}</a>
