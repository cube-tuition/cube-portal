 ...
 *
 * `enrolment_meta` holds the draft enrolment table's planning fields — who has
 * been confirmed with the parent / the teacher, and a note — keyed
 * "<class_id>|<student_id>". It stays in the draft; Apply to live ignores it.
 */
...
export async function loadDraft(id) {
  ...
  return {
    ...
    enrolment_meta: data.enrolment_meta && typeof data.enrolment_meta === 'object' ? data.enrolment_meta : {},
  }
}
...
export async function saveDraft(id, { entries, hiddenIds, name, enrolmentMeta } = {}) {
  if (!id) return
  const patch = { updated_at: new Date().toISOString() }
  if (entries != null)  patch.entries = entries.map(pickEntry)
  if (hiddenIds != null) patch.hidden_ids = hiddenIds
  if (name != null)     patch.name = name
  if (enrolmentMeta != null) patch.enrolment_meta = enrolmentMeta
