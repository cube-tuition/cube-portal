...
// Subject families for the resource-hub scope (?subject=Maths|English|…).
// The Maths family spans the junior subject plus the senior variants; every
// page that scopes by subject reads these, so a new subject is added once.
...
export const SCOPE_LABEL = {
  Maths: 'Mathematics', English: 'English', Chemistry: 'Chemistry', Science: 'Science',
}

/*
 * School stages. Skills are shared across a whole stage rather than owned by one
 * year: Years 5-6 draw on one list, Years 7-12 on another, so the same skill
 * does not have to be recreated for every year that teaches it.
 */
export const STAGES = ['primary', 'high']
export const STAGE_LABEL = { primary: 'Primary · Years 5-6', high: 'High school · Years 7-12' }
export const stageOfYear = (year) => (Number(year) <= 6 ? 'primary' : 'high')

/** The family a subject row belongs to ('Adv Maths' → 'Maths'). */
export function familyOfSubject(name) {
  const n = String(name || '')
  return Object.keys(SUBJECT_FAMILIES).find((fam) => SUBJECT_FAMILIES[fam].includes(n)) || n
}

/*
 * How a subject row reads on screen. A row is a (year, stream) pair, and for the
 * junior years the stream IS the family — so "Maths" at Year 8 is just "Year 8",
 * while Year 11's streams need naming: "Year 11 Ext 1".
 */
export function subjectYearLabel(subject, family) {
  const name = String(subject?.name || '')
  const variant = (name && name !== family && (SUBJECT_FAMILIES[family] || []).includes(name))
    ? ' ' + name.replace(/\s*Maths\s*/i, ' ').trim()
    : ''
  return `Year ${subject?.year_level}${variant}`
}

/** The key a skill list is stored under: one list per family per stage. */
export const skillScopeKey = (family, stage) => `${family}|${stage}`
export const skillScopeOfSubject = (subject) =>
  skillScopeKey(familyOfSubject(subject?.name), stageOfYear(subject?.year_level))

/*
 * Difficulty scale, 1-4. There used to be a fifth level, "Extension", carried by
 * 106 questions from bulk inserts made against a 1-5 scale; those have been
 * folded into 4 and the level retired, so the scale the pickers offer and the
 * scale the data uses are now the same one.
 */
...
// The top of the scale, for averages.
...
/*
 * The label a question part is shown under: its position, always alphabetical.
 *
 * Labels used to be typed by hand and stored per part, which went stale the
 * moment a part was inserted or removed — deleting (b) left (a) and (c) on the
 * page. They are derived from order instead, so the list always reads a, b, c…
 * Beyond 26 parts it falls back to a number rather than running off the end of
 * the alphabet, which the old inline 'abcdefgh' lookup did at the 9th part.
 */
export const partLabel = (i) => 'abcdefghijklmnopqrstuvwxyz'[i] || String(i + 1)

/*
 * A question's total marks.
 *
 * For a multi-part question the parts are the source of truth: the author marks
 * (a), (b), (c) and the total follows. A typed total could disagree with them —
 * add a part, forget the header, and the paper's section total no longer
 * matches the marks printed beside the parts. Editors keep `marks` in step for
 * anything that reads the column directly, and every reader goes through here
 * so older rows saved before that (blank or stale) still total correctly.
 */
export const sumPartMarks = (parts) =>
  (parts || []).reduce((sum, p) => sum + (Number(p.marks) || 0), 0)

export function questionTotalMarks(q) {
  if (!q) return 0
  // A multiple-choice question has no parts; an unmarked one is worth 1.
  if (q.qtype === 'mcq') return q.marks == null ? 1 : Number(q.marks) || 0
  const parts = q.qbank_question_parts || q.parts || []
  if (parts.length) return sumPartMarks(parts)
  return Number(q.marks) || 0
}

/*
 * Split a list of worksheets into subject groups, for the Additional Questions
 * master database.
 *
 * A worksheet filed under a curriculum topic already knows its subject; an
 * unfiled one does not, so the caller passes `subjectOf` to work one out
 * however it can (the bank taxonomy of the questions on the sheet). Whatever it
 * cannot place goes under `unsorted`, which always sorts last — those are the
 * sheets that need attention, not a subject.
 *
 * Order within a group is preserved, so the caller's own sort (year, then most
 * recent) still holds. `rank` orders the groups; it defaults to alphabetical.
 */
export function groupWorksheetsBySubject(worksheets, subjectOf, { unsorted = 'Unsorted', rank = () => 0 } = {}) {
  const groups = new Map()
  for (const ws of worksheets || []) {
    const su = subjectOf(ws) || unsorted
    if (!groups.has(su)) groups.set(su, [])
    groups.get(su).push(ws)
  }
  return [...groups.entries()].sort((a, b) =>
    (a[0] === unsorted) - (b[0] === unsorted)
    || rank(a[0]) - rank(b[0])
    || a[0].localeCompare(b[0]))
}

/*
 * The question bank's subtopics for a CURRICULUM topic row ({ year, subject,
 * name } from `topics`).
 *
 * The two trees are separate: worksheets are filed under a curriculum topic so
 * they show up on its Materials page, while subtopics only exist in the bank.
 * They meet by name — the same match the Materials topic page makes when it
 * counts the bank's questions — so a worksheet can name the subtopic it covers
 * without a second tree being invented for it. Returns [] when either side is
 * missing, which the callers treat as "this topic has no subtopics to offer".
 */
export function qbankSubtopicsForCurriculumTopic(tax, topic) {
  if (!tax || !topic) return []
  const eq = (a, b) => String(a ?? '').trim().toLowerCase() === String(b ?? '').trim().toLowerCase()
  const subject = (tax.subjects || []).find(
    (s) => String(s.year_level) === String(topic.year) && eq(s.name, topic.subject))
  if (!subject) return []
  const qTopic = (tax.topicsBySubject?.[subject.id] || []).find((t) => eq(t.name, topic.name))
  if (!qTopic) return []
  return tax.subtopicsByTopic?.[qTopic.id] || []
}
...
//           subtopicsByTopic, skillsByScope, skillsBySubject }.
// Skills are a STAGE-level dimension (family + stage); topic_id/subtopic_id are
...
export async function fetchTaxonomy() {
  ...
  // Skills are keyed by (family, stage). skillsBySubject then points EVERY
  // subject row at its stage's list, so Year 8 and Year 9 receive the same
  // array — which is what "shared across the stage" means — and the four
  // pickers that read skillsBySubject[subjectId] keep working untouched.
  const subjectById = Object.fromEntries(subjects.map((su) => [su.id, su]))
  const skillsByScope = {}
  skills.forEach((s) => {
    // A skill written before the stage columns existed — or by a client still
    // running the old code — carries only subject_id. Derive its scope from
    // that subject rather than dropping it: a skill that silently disappears
    // is worse than one that lands in the right stage by inference.
    const key = (s.family && s.stage)
      ? skillScopeKey(s.family, s.stage)
      : (subjectById[s.subject_id] ? skillScopeOfSubject(subjectById[s.subject_id]) : null)
    if (key) (skillsByScope[key] ||= []).push(s)
  })
  const skillsBySubject = {}
  subjects.forEach((su) => {
    skillsBySubject[su.id] = skillsByScope[skillScopeOfSubject(su)] || []
  })
  return {
    subjects, topics, subtopics, skills,
    topicsBySubject, subtopicsByTopic, skillsByScope, skillsBySubject,
  }
}
 ...
 */
/*
 * Read a whole table. PostgREST caps a select at its max-rows setting (1000)
 * and does so silently — no error, just a short list — so once the question
 * bank passed that size, whole-bank reads began quietly losing their newest
 * rows. `build` must return a fresh query builder each call, since a builder
