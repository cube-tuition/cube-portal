...
// branded email shell. `highlight` is the special-offer line, rendered as a
// prominent banner near the top. Pure function — safe on client and server.
export function buildCourseOfferEmailHtml(body, vars = {}, highlight = '') {
  ...
  const banner = hl ? `
      <div style="background:linear-gradient(120deg,#F59E0B 0%,#FBBF24 100%);border-radius:14px;padding:20px 24px;margin-bottom:30px;text-align:center;box-shadow:0 4px 14px rgba(245,158,11,0.28);">
        <p style="margin:0 0 6px;font-size:11px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#7C2D12;">★ Special offer</p>
        <p style="margin:0;font-size:20px;font-weight:800;line-height:1.35;color:#1c1c1c;">${richInline(hl, vars)}</p>
      </div>` : ''
  return `<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f0f4ff;">
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:600px;margin:32px auto;padding:32px 24px;color:#2A2035;background:#ffffff;border-radius:12px;box-shadow:0 2px 16px rgba(6,46,99,0.08);">
      ...
      <div style="margin-top:26px;background:#F0F4FF;border:1px solid #DEE7FF;border-radius:12px;padding:18px 20px;">
        <p style="margin:0 0 6px;font-size:12px;font-weight:700;color:#062E63;text-transform:uppercase;letter-spacing:0.6px;">${MULTI_COURSE_NOTE.heading}</p>
        <p style="margin:0;font-size:14px;line-height:1.7;color:#2A2035;">${MULTI_COURSE_NOTE.body}</p>
      </div>
    </div>
  </body></html>`
}
