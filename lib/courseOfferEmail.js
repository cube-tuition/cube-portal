...
// The call to action: the website's free-trial form, as a button.
//
// "Just reply to this email" asks a parent to compose something, and a reply
// lands in a mailbox rather than in Trials — the form puts them straight into
// the trial pipeline with the details we need. Written as {{trial_button}} on
// its own line in the body, so the author chooses where it sits.
const TRIAL_BUTTON_LABEL = 'Book a free trial →'
const trialButton = () => `
      <div style="text-align:center;margin:28px 0 24px;">
        <a href="${FREE_TRIAL_URL}" style="display:inline-block;background:#062E63;color:#ffffff;text-decoration:none;font-size:15px;font-weight:700;letter-spacing:0.4px;padding:14px 34px;border-radius:10px;">${TRIAL_BUTTON_LABEL}</a>
        <p style="margin:10px 0 0;font-size:12px;color:#5b7bc4;">Takes a minute — we'll be in touch to arrange a time.</p>
      </div>`
...
export function buildCourseOfferEmailHtml(body, vars = {}, highlight = '') {
  ...
  for (const block of filled.split(/\n\n+/)) {
    ...
    if (block.trim() === '{{trial_button}}') { bodyHtml += trialButton(); continue }
    bodyHtml += `<p style="margin:0 0 16px 0;line-height:1.7;">${esc(block).replace(/\n/g, '<br/>').replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')}</p>`
  }
