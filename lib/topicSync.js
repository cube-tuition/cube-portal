...
/**
 * What is attached to this topic on each side, so the confirmation can say what
 * a delete will do before it does it.
 */
...
/*
 * Nothing blocks a topic delete any more — it only ever removes the topic and
 * the taxonomy under it, never the work filed against it.
 *
 * Questions: qbank_questions.topic_id, .subtopic_id and .skill_id are all
 * ON DELETE SET NULL, so a question keeps its stem, parts and images and simply
 * loses its tags. It is then listed under the bank's Untagged tab.
 *
 * Workbooks: `booklets.topic` is a plain string with no foreign key, so
 * mirrorDelete clears it explicitly. Those workbooks move into the Master
 * Database's "No topic assigned" group, where each one can be retagged from the
 * topic picker on its row.
 */
...
/*
 * Delete from both sides, keeping everything filed under the topic.
 *
 * Subtopics and skills go with it — they are part of the same taxonomy — but the
 * questions tagged to them are kept and untagged by the foreign keys (see
 * migrations/20260905_qbank_delete_keeps_questions.sql). Workbooks have no such
 * key, so their topic is cleared here, in the same operation: leaving the string
 * behind would point them at a topic that is no longer in the list.
 */
export async function mirrorDelete(year, subject, name) {
  const n = clean(name)
  const pair = await findPair(year, subject, n)
  // Untag the workbooks BEFORE removing the topic, so a failure here leaves the
  // topic in place rather than stranding them.
  const { error: bErr } = await supabase.from('booklets').update({ topic: null })
    .eq('year', year).eq('subject', subject).eq('topic', n)
