 ...
 *
 * Questions do NOT block. qbank_questions.topic_id, .subtopic_id and .skill_id
 * are all ON DELETE SET NULL, so deleting a topic keeps every question, its
 * parts and its images, and simply leaves it untagged — it then shows up under
 * the bank's "no topic" filter, ready to be re-tagged. Workbooks still block:
 * `booklets.topic` is a plain string, so a delete would leave those workbooks
 * pointing at a topic that no longer exists in the list, with nothing to
 * re-tag them from.
 */
export const deleteBlocked = (impact) => impact.booklets > 0
 ...
 *
 * Subtopics and skills under the topic go with it (they are part of the same
 * taxonomy), but the questions tagged to them are kept and untagged by the
 * foreign keys — see migrations/20260905_qbank_delete_keeps_questions.sql.
 */
