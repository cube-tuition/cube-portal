 ...
 *   noHomework      true for a student whose classes carry no homework grade
 *                   (Year 11/12) — the homework component is dropped and its
 *                   weight shared over the rest
...
export function engagementScore(i) {
  ...
  // Year 11/12 are not given a homework grade at all, so scoring them out of a
  // component they cannot earn would mark them down for the centre's own
  // policy. Their 30 points are shared across the parts they DO have, in the
  // same proportions, leaving the score out of 100 either way.
  const used = i.noHomework ? SCORE_PARTS.filter((p) => p.key !== 'homework') : SCORE_PARTS
  const weight = used.reduce((a, p) => a + p.weight, 0)
  const total = Math.round(used.reduce((a, p) => a + parts[p.key] * p.weight, 0) / weight * 100)
  return { total, parts, noHomework: !!i.noHomework }
}
