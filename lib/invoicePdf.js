...
const GREEN  = [6, 95, 70]      // #065F46 — paid state
...
export async function generateInvoicePdf(inv, termName, termDates) {
  ...
  // Cash invoices print as a plain consumer INVOICE with no GST breakdown
  // (tax invoices are only required on request for B2C; GST is still inside
  // the price and reported on the BAS as normal). Bank invoices stay full
  // TAX INVOICE format.
  const isCash = inv.payment_method === 'cash' ||
    (inv.line_items || []).some(l => l?.type === 'discount' && /^Cash discount/i.test(l.reason || ''))

  doc.setFont('helvetica', 'bold')
  doc.setFontSize(21)
  doc.setTextColor(...NAVY)
  doc.text(isCash ? 'INVOICE' : 'TAX INVOICE', L, 22)
  ...
  // Title width varies (INVOICE vs TAX INVOICE) — measure it so status chips
  // sit just after the text either way.
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(21)
  const chipX = L + doc.getTextWidth(isCash ? 'INVOICE' : 'TAX INVOICE') + 4

  if (isDraft) {
    // status chip next to title
    doc.setDrawColor(...NAVY)
    doc.setLineWidth(0.4)
    doc.roundedRect(chipX, 16.2, 18, 7, 1.4, 1.4, 'S')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(7.5)
    doc.setTextColor(...NAVY)
    doc.text('DRAFT', chipX + 9, 20.8, { align: 'center', charSpace: 0.4 })
  } else if (inv.payment_status === 'paid') {
    // green PAID chip — the payment date itself prints under the totals band
    doc.setFillColor(...GREEN)
    doc.roundedRect(chipX, 16.2, 16, 7, 1.4, 1.4, 'F')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(7.5)
    doc.setTextColor(...WHITE)
    doc.text('PAID', chipX + 8, 20.8, { align: 'center', charSpace: 0.4 })
  }
  ...
  const facts = [
    [isPaid ? 'Amount paid' : 'Amount due', money(inv.total), true],
    isPaid && inv.paid_date
      ? ['Paid on',  fmtDateLong(inv.paid_date), false]
  ...
  allLines.forEach(l => {
    ...
    doc.setFontSize(9)
    doc.setTextColor(...GREY)
    if (isEnrol) doc.text('1', cQty, textY, { align: 'right' })
  ...
  // Amount due band — paid invoices flip to a green AMOUNT PAID band with the
  // payment date printed underneath.
  ty += 5
  doc.setFillColor(...(isPaid ? GREEN : NAVY))
  doc.roundedRect(tx - 4, ty - 5.2, R - tx + 9, 11.6, 2, 2, 'F')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  doc.setTextColor(...WHITE)
  doc.text(isPaid ? 'AMOUNT PAID' : 'AMOUNT DUE', tx, ty + 1.6, { charSpace: 0.4 })
  doc.setFontSize(13.5)
  doc.text(money(totalIncGst), R, ty + 1.8, { align: 'right' })
  if (isPaid && inv.paid_date) {
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8.5)
    doc.setTextColor(...GREEN)
    doc.text(`Paid on ${fmtDateLong(inv.paid_date)}`, R, ty + 11.5, { align: 'right' })
  }
...
// routes (send-term-start-emails) that email the PDF
