...
export async function generateInvoicePdf(inv, termName, termDates) {
  ...
  const subBits = [inv.invoice_number, termName, termDates].filter(Boolean).join('   ·   ')
  if (subBits) doc.text(subBits, L, 28.5)

  if (isDraft) {
    // status chip next to title
    doc.setFillColor(...PANEL)
    doc.roundedRect(L + 52, 16.2, 18, 7, 1.6, 1.6, 'F')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8)
    doc.setTextColor(...NAVY)
    doc.text('DRAFT', L + 61, 20.8, { align: 'center' })
  }
  ...
  // ── Key facts strip (panel) ─────────────────────────────────────────────────
  const stripH = 20
  doc.setFillColor(...PANEL)
  doc.roundedRect(L, y, R - L, stripH, 2.5, 2.5, 'F')
  const facts = [
  ...
  facts.forEach(([label, value, hero], i) => {
    ...
    doc.setFontSize(6.8)
    doc.setTextColor(...GREY)
    doc.text(label.toUpperCase(), cx, y + 7.4, { charSpace: 0.5 })
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(hero ? 14 : 10.5)
    doc.setTextColor(...(hero ? NAVY : INK))
    doc.text(value, cx, y + 15)
  })
  y += stripH + 12
  ...
  const drawTableHeader = () => {
    doc.setFillColor(...NAVY)
    doc.roundedRect(L, y - 5.4, R - L, 8.4, 1.8, 1.8, 'F')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(7.2)
    doc.setTextColor(...WHITE)
    doc.text('DESCRIPTION', cDesc, y, { charSpace: 0.5 })
    doc.text('PRICE', cPrice, y, { align: 'right', charSpace: 0.5 })
    doc.text('GST', cTax, y, { align: 'right', charSpace: 0.5 })
  ...
  allLines.forEach(l => {
    ...
    const sub = isEnrol
      ? [l.day, l.start_time].filter(Boolean).join(' ')
      : null

    const amt = Number(l.amount) || 0
    const price = Number(l.unit_price ?? l.amount) || 0

    const titleLines = doc.splitTextToSize(title, cPrice - cDesc - 14)
    ...
    doc.text(money(price), cPrice, textY, { align: 'right' })
    doc.setTextColor(...GREY)
    doc.setFontSize(8)
    doc.text(amt !== 0 ? '10%' : '—', cTax, textY, { align: 'right' })
    doc.setFontSize(9)
    doc.setTextColor(...(amt < 0 ? GREY : INK))
  ...
  doc.setFillColor(...PANEL)
  doc.roundedRect(L, boxTop, instrW, instrH, 2.5, 2.5, 'F')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(7)
  doc.setTextColor(...NAVY)
  ...
  totRow('Includes GST (10%)', money(gst))
