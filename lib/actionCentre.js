...
const STALE_DAYS = 3
const TRANSITION_WINDOW_DAYS = 21
...
export async function runActionChecks() {
  ...
  const [trialsRes, invoicesRes, lessonsRes, attendanceRes, shiftsRes, studentsRes, guardiansRes, creditsRes, enrolRes, orphanGuardRes] = await Promise.all([
    ...
    term ? supabase.from('enrolments').select('id, status, next_term_status, class_id, classes!inner(term_id)').eq('status', 'active').eq('classes.term_id', term.id) : { data: [] },
  ...
  // ── 5. Term transition window ───────────────────────────────────────────────
  if (term?.end_date) {
    const daysToEnd = Math.ceil((new Date(term.end_date) - new Date(today)) / dayMs)
    if (daysToEnd <= TRANSITION_WINDOW_DAYS && daysToEnd > -7) {
      const pending = (enrolRes.data ?? []).filter(e => (e.next_term_status ?? 'pending') === 'pending')
      add(daysToEnd <= 7 ? 'red' : 'amber', '🔄', pending.length,
        `families undecided for next term (term ends in ${daysToEnd}d)`,
        'Confirm continuations early — it’s far cheaper than re-acquiring.', '/tutor/transition')
    }
  }
