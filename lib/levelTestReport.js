...
export async function exportLevelTestReport(args, { preview = false, base64 = false, debug = false } = {}) {
  ...
  try {
    ...
    const dataUrl = await nodeToJpeg(node, { quality: 0.95, width: PAGE_W, height: capH, backgroundColor: '#ffffff', pixelRatio: 2 })
    const pdf = new jsPDF({ orientation: 'p', unit: 'mm', format: 'a4' })
    const pdfW = pdf.internal.pageSize.getWidth()
    const pdfH = pdf.internal.pageSize.getHeight()
    const scale = pdfW / PAGE_W
    const fullH = capH * scale
    let cutsOut = [0]
    if (fullH <= pdfH + 0.5) {
      pdf.addImage(dataUrl, 'JPEG', 0, 0, pdfW, fullH)
    } else {
      /* Page breaks at BLOCK boundaries, never through one. The naive slicer
         cut at fixed heights, splitting a topic bar or a strengths box across
         two pages. Blocks are the root's children; anything taller than a
         page (a many-topic test section) is exploded into its own children,
         recursively, until pieces fit. A cut lands just above the first block
         that would cross the page edge, and the unused remainder of each page
         is masked white so the next page's content can't bleed into it. */
      const rootTop = node.getBoundingClientRect().top
      const candidates = []
      const collect = (el) => {
        const r = el.getBoundingClientRect()
        if (r.height <= 0) return
        if (r.height > PAGE_H - 140 && el.children.length > 1) { for (const kid of el.children) collect(kid); return }
        candidates.push({ top: r.top - rootTop, bottom: r.bottom - rootTop })
      }
      for (const el of node.children) {
        if (getComputedStyle(el).position === 'absolute') continue   // the footer rides the image bottom
        collect(el)
      }
      ...
      for (const b of candidates) {
        if (b.bottom <= pageEnd + 0.5) { prevBottom = Math.max(prevBottom, b.bottom); continue }
        const cur = cuts[cuts.length - 1]
        // Cut just above the crossing block, clamped INSIDE the gap after the
        // previous block — a fixed back-off overshot a tight gap and clipped
        // the block above. If the block started at the very page top there is
        // no gap to use; hard-slice as a last resort.
        const gapCut = Math.min(b.top - 2, Math.max(prevBottom + 2, b.top - 26))
        const cutAt = b.top - cur > 60 && gapCut > cur + 60 ? gapCut : pageEnd
        cuts.push(cutAt)
        pageEnd = cutAt + PAGE_H
        while (b.bottom > pageEnd + 0.5) { cuts.push(pageEnd); pageEnd += PAGE_H }
        prevBottom = Math.max(prevBottom, b.bottom)
      }
      while (capH > pageEnd + 0.5) { cuts.push(pageEnd); pageEnd += PAGE_H }
      cutsOut = cuts

      cuts.forEach((y, i) => {
        if (i) pdf.addPage()
        pdf.addImage(dataUrl, 'JPEG', 0, -y * scale, pdfW, fullH)
        const winH = (Math.min(cuts[i + 1] ?? capH, y + PAGE_H) - y) * scale
        if (winH < pdfH - 0.5) {
          pdf.setFillColor(255, 255, 255)
          pdf.rect(0, winH, pdfW, pdfH - winH + 1, 'F')
        }
      })
    }
    if (base64) return { base64: pdf.output('datauristring').split(',')[1], filename, ...(debug ? { cuts: cutsOut, capH } : {}) }
    if (preview) return { url: URL.createObjectURL(pdf.output('blob')), filename, ...(debug ? { cuts: cutsOut, capH } : {}) }
    pdf.save(filename)
    return { filename }
  } finally {
