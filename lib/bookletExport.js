 ...
 */
import { coverHtml, BOOKLET_CSS, WATERMARK_SVG, footerHtml, bookletRenderItems } from './bookletRender'
...
export async function exportBookletPdf({ meta = {}, blocks = [], solutions = false, preview = false }) {
  ...
  pages.forEach((p, i) => {
    ...
    const label = (quizStartPage >= 0 && i >= quizStartPage) ? 'Mathematics Revision Quiz'
      : (hwStartPage >= 0 && i >= hwStartPage) ? 'Mathematics Homework'
      : 'Mathematics Booklet'
