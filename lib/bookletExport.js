      ...
      if (!it.chunks && el.lastElementChild
        ...
        if (rest) {
          if (el.querySelectorAll('.bk-line').length >= MIN_FLOW_LINES) {
            ...
            // Too few lines landed here — put the tail back and let the block
            // move whole. Every line is a sibling under .bk-lines, and the head
            // always keeps at least one, so the container is still there.
            const host = el.querySelector('.bk-lines')
            if (host) rest.querySelectorAll('.bk-line').forEach((l) => host.appendChild(l))
          }
        }
      }
      if (flowed) {
