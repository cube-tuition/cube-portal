 ...
 */
import { coverHtml, blockHtml, BOOKLET_CSS, WATERMARK_SVG, footerHtml } from './bookletRender'
...
export async function exportBookletPdf({ meta = {}, blocks = [], solutions = false, preview = false }) {
  ...
  cover.innerHTML = coverHtml(meta, { solutions })
  stage.appendChild(cover); pages.push(cover)

  // Content pages — paginate blocks, moving an overflowing block to a new page.
  let cur = newContentPage(stage); pages.push(cur.page)
  let countOnPage = 0
  let qNum = 0
  for (const b of blocks) {
    if (b.type === 'question' || b.type === 'mcq') qNum++
    // Append the block's REAL root element (a .bk-block) directly, so blocks are
    ...
    tmp.innerHTML = blockHtml(b, { solutions, qNum })
  ...
  // Footers (page numbers start at 1 on the cover).
  pages.forEach((p, i) => {
    if (i === 0) return // cover has no footer
    const f = document.createElement('div')
    f.innerHTML = footerHtml(i + 1)
