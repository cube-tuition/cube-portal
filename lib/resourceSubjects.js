...
export const SUBJECTS = {
  ...
  science: {
    ...
    value: 'Science',
    icon: '🔬',
    blurb: 'Everything for junior Science classes — curriculum, questions, materials and exams.',
    accent: '#B45309',
    tint: '#FFFBEB',
    border: '#FDE68A',
  },
 ...
 * the two together. Standard Maths gets no tab — CUBE doesn't run it as a
 * materials course, so listing it was noise.
 *
...
export const COURSE_TABS = {
  maths: [
    ...
    { key: '11-adv',  label: 'Year 11 Adv',       year: 11, subject: 'Adv Maths' },
    { key: '11-ext1', label: 'Year 11 Ext 1',     year: 11, subject: 'Ext 1 Maths' },
    { key: '12-adv',  label: 'Year 12 Adv',       year: 12, subject: 'Adv Maths' },
...
export const MATERIAL_AREAS = (subjectValue, tab = null) => {
  ...
  return [
    // adminOnly: the master DB page itself turns tutors away, so the card
    // would be a dead link on their portal — hide it instead.
    { label: 'WB Master Database', icon: '📓', adminOnly: true,
      href: `/tutor/booklets/master?subject=${subjectValue}${scope}`,
      desc: 'The master workbook database and the workbook builder.' },
    { label: 'AQ Master Database', icon: '📝', adminOnly: true,
...
export const AREAS = (subjectValue, slug) => [
  ...
  { label: 'Holiday Courses', icon: '🏖️', href: `/tutor/booklets/holiday?subject=${subjectValue}`,
    desc: 'Holiday programmes — the workbook planned for each day of a course.' },
...
export function bookletPdfs(b) {
  ...
  return paths.map((path, i) => ({
    path,
    isSolutions: flags[i],
    // The name the file downloads as, when the row has one stored.
    filename: b?.pdf_filenames?.[i] ? `${b.pdf_filenames[i]}.pdf` : null,
...
/*
 * PDF button colours. The solutions copy is tinted differently from the student
 * copy so the two are never confused at a glance on a workbook row.
 */
export const PDF_BUTTON_STYLE = (isSolutions) => isSolutions
