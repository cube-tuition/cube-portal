import { createClient } from '@supabase/supabase-js'
import { createHash, timingSafeEqual } from 'node:crypto'

/*
 * Server-side API auth helper.
 *
 * Privileged API routes run with the service-role key (which bypasses RLS), so
 * they MUST verify the caller themselves. Call requireApiRole() at the top of a
 * route handler: it reads the caller's Supabase JWT from the
 * `Authorization: Bearer <token>` header, verifies it, and checks that
 * app_metadata.role (set server-side only) is allowed.
 *
 *   const auth = await requireApiRole(req, ['admin', 'director'])
 *   if (!auth.ok) return Response.json({ error: auth.error }, { status: auth.status })
 *
 * Pass allowedRoles = null (or omit) to accept ANY authenticated user.
 */

let _admin
function admin() {
  if (!_admin) {
    _admin = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL,
      process.env.SUPABASE_SERVICE_ROLE_KEY,
    )
  }
  return _admin
}

export async function requireApiRole(request, allowedRoles = null) {
  const authHeader = request.headers.get('authorization') || ''
  if (!authHeader.startsWith('Bearer ')) {
    return { ok: false, status: 401, error: 'Missing auth token' }
  }
  const token = authHeader.slice(7)

  const { data: { user }, error } = await admin().auth.getUser(token)
  if (error || !user) {
    // The caller's session died server-side (revoked, or a refresh-token race
    // between tabs). Nothing they retry will work until they re-authenticate,
    // so the message must say that — "invalid token" reads like OUR bug.
    return { ok: false, status: 401, error: 'Your session has expired — log out and back in, then try again.' }
  }

  const role = user.app_metadata?.role ?? 'student'
  if (allowedRoles && allowedRoles.length && !allowedRoles.includes(role)) {
    return { ok: false, status: 403, error: 'Forbidden' }
  }

  return { ok: true, user, role }
}

/*
 * Cron-job auth for the Airtable sync routes, which run with the service-role
 * key and are triggered by Vercel cron rather than by a signed-in user.
 *
 *   const auth = requireCronSecret(request)
 *   if (!auth.ok) return Response.json({ error: auth.error }, { status: auth.status })
 *
 * Compares SHA-256 digests with timingSafeEqual: hashing first gives both sides
 * a fixed length (timingSafeEqual throws on length mismatch) and keeps the
 * comparison constant-time, so the secret can't be recovered byte-by-byte.
 * Fails closed when CRON_SECRET is unset — otherwise a missing env var would
 * make the literal header `Bearer undefined` a valid credential.
 */
export function requireCronSecret(request) {
  const secret = process.env.CRON_SECRET
  if (!secret) {
    return { ok: false, status: 500, error: 'CRON_SECRET is not configured' }
  }
  const digest = (s) => createHash('sha256').update(s).digest()
  const provided = digest(request.headers.get('authorization') || '')
  const expected = digest(`Bearer ${secret}`)
  if (!timingSafeEqual(provided, expected)) {
    return { ok: false, status: 401, error: 'Unauthorized' }
  }
  return { ok: true }
}
