...
export function questionChunksHtml(b, ctx) {
  ...
  return starts.map((from, k) => {
    ...
    const brk = (from >= 1 && parts[from].pageBreakBefore) ? ' data-break="1"' : ''
    return from === 0
...
// True when a multi-part question asks to break before one of its parts, so the
// paginators must always split it into chunks (not just on overflow).
export function hasForcedPartBreak(b) {
  return b?.type === 'question' &&
    (b.parts || []).length >= 2 && (b.parts || []).some((p, i) => i >= 1 && p.pageBreakBefore)
}
