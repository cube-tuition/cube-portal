...
function tableHtml(b, embedded = false) {
  ...
  const bodyRow = (cells) => `<tr>${cells.map((c, i) =>
    (headerCol && i === 0) ? cell(c, 'th', 'row') : cell(c, 'td')).join('')}</tr>`
  const head = b.headerRow ? headRow(rows[0]) : ''
  const body = (b.headerRow ? rows.slice(1) : rows).map(bodyRow).join('')
  const w = Number(b.width)
  const hasW = Number.isFinite(w) && w > 0 && w < 100
...
/* Booklet type is loaded as WEB fonts, never from the local system. The body
   used to ask for Avenir Next, a macOS font with no @font-face anywhere: the
   PDF export rasterises pages in whichever browser publishes, and html-to-image
   can only embed fonts the CSS declares. Publishing from a machine without
   Avenir silently fell back to Segoe UI, changing the type AND the metrics,
   which overflowed the fixed-height page and cropped the footer. */
@import url('https://fonts.googleapis.com/css2?family=Jost:wght@300;400;500;600;700&family=Nunito+Sans:opsz,wght@6..12,300;6..12,400;6..12,600;6..12,700;6..12,800&display=swap');
.bk-root{ --ink:#1c1c1c; --blue:#5b7bc4; --grey:#e7e8ea; --rule:#333; font-family:'Nunito Sans','Segoe UI',system-ui,Helvetica,Arial,sans-serif; color:var(--ink); }
...
.bk-cv{ position:relative; width:794px; height:1123px; background:#F5F6F8; overflow:hidden; box-sizing:border-box; font-family:'Jost','Nunito Sans','Segoe UI',system-ui,Helvetica,Arial,sans-serif; color:#364466; }
...
.bk-tnotes-title{ margin:0 0 2px; font-size:11.5px; font-weight:600; letter-spacing:.05em; text-transform:uppercase; color:#0E7A5F; font-family:'Nunito Sans','Segoe UI',system-ui,sans-serif; }
...
.katex{ font-size:1em; }
/* ── Level-test cover ──────────────────────────────────────────────────────── */
.bk-lt{ position:relative; width:794px; height:1123px; background:#fff; box-sizing:border-box; padding:64px 72px; font-family:'Nunito Sans','Segoe UI',system-ui,Helvetica,Arial,sans-serif; color:#1c1c1c; }
