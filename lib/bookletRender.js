...
// Compile an equation in x (e.g. "y = x^2 - 2", "2x + 1", "12/x", "sqrt(x)")
// into a safe function. Supports + - * / ^, parentheses, implicit
// multiplication (2x, 2(x+1)), sqrt/abs/sin/cos/tan and pi. Returns null if
// the input doesn't parse.
function compileEquation(src) {
  let e = String(src ?? '').toLowerCase().replace(/\s+/g, '')
  if (!e) return null
  e = e.replace(/^y=/, '').replace(/^f\(x\)=/, '')
  ...
  if (!/^[0-9x+\-*/().^#saiot]*$/.test(e)) return null
  // Implicit multiplication: 2x, 2(…), x(…), )(, )x, 2sqrt(…) etc.
  e = e.replace(/(\d|x|\))(?=(x|\(|#))/g, '$1*')
  e = e.replace(/\)(?=\d)/g, ')*')
  e = e.replace(/\^/g, '**')
  // Maths convention -x^2 = -(x^2); JS rejects bare "-x**2", so wrap it.
  e = e.replace(/-(x|\d+\.?\d*)\*\*(\([^()]*\)|x|\d+\.?\d*)/g, '-($1**$2)')
  e = e.replace(/#s/g, 'Math.sqrt').replace(/#a/g, 'Math.abs').replace(/#i/g, 'Math.sin').replace(/#o/g, 'Math.cos').replace(/#t/g, 'Math.tan')
  try {
    // eslint-disable-next-line no-new-func
    const f = new Function('x', `"use strict"; return (${e});`)
    f(1); f(0)   // smoke-test — throws on stray identifiers
    return f
  } catch { return null }
}
...
function cartesianSvg(b) {
  ...
  for (const cv of parseCurveRows(b.lines)) {
    ...
    if (cv.label && end) s += `<text x="${X(end[0]) - 4}" y="${Y(end[1]) - 7}" font-size="11.5" font-weight="bold" fill="#325099" text-anchor="end">${esc(cv.label)}</text>`
    // Optional intercept labels: y-intercept at f(0), x-intercepts found by
    // sign-change scan + bisection.
    if (showNums) {
      ...
      if (Number.isFinite(y0) && y0 >= yMin && y0 <= yMax) {
        s += `<circle cx="${X(0)}" cy="${Y(y0)}" r="3.4" fill="#0F766E"/>`
        s += `<text x="${X(0) + 7}" y="${Y(y0) - 6}" font-size="11" font-weight="bold" fill="#0F766E">(0, ${fmtN(y0)})</text>`
      }
      if (0 >= yMin && 0 <= yMax) {
        const roots = []
        let px = xMin, py = evalAt(xMin)
        for (let i = 1; i <= N; i++) {
          const x = xMin + (i / N) * (xMax - xMin)
          const y = evalAt(x)
          if (Number.isFinite(py) && Number.isFinite(y)) {
            if (py === 0 && i === 1) roots.push(px)
            if ((py < 0 && y > 0) || (py > 0 && y < 0) || y === 0) {
              // bisect for a clean root (skip if it's just an asymptote jump)
              let a = px, bx = x, fa = py
              for (let k = 0; k < 30; k++) {
                const m = (a + bx) / 2, fm = evalAt(m)
                if (!Number.isFinite(fm)) break
                if ((fa < 0 && fm < 0) || (fa > 0 && fm > 0)) { a = m; fa = fm } else bx = m
              }
              const r = (a + bx) / 2
              if (Math.abs(evalAt(r)) < 0.05 && !roots.some(q => Math.abs(q - r) < (xMax - xMin) / 100)) roots.push(r)
            }
          }
          px = x; py = y
        }
        for (const r of roots) {
          ...
          s += `<circle cx="${X(r)}" cy="${Y(0)}" r="3.4" fill="#0F766E"/>`
          s += `<text x="${X(r) + 7}" y="${Y(0) + 16}" font-size="11" font-weight="bold" fill="#0F766E">(${fmtN(r)}, 0)</text>`
        }
      }
    }
  }
...
export function blockHtml(b, ctx) {
  switch (b.type) {
    ...
    case 'mathobj': {
      ...
      // Float position: the following block's text wraps around the object,
      // same as floated images (its clearfix contains the float's height).
      if (b.pos === 'left' || b.pos === 'right') {
        return `<div class="bk-block bk-img-float bk-img-float-${b.pos} bk-mathobj-floatblock" style="width:${wPct}%">${mathObjectSvg(b)}</div>`
      }
      return `<div class="bk-block bk-mathobj"><div class="bk-mathobj-inner" style="width:${wPct}%">${mathObjectSvg(b)}</div></div>`
    }
    case 'image': {
