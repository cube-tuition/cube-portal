...
export function questionChunksHtml(b, ctx) {
  ...
  return parts.map((p, i) => {
    ...
    // A part (other than the first) marked "start on a new page" tags its chunk
    // so the paginators force a page break before it.
    const brk = (i >= 1 && p.pageBreakBefore) ? ' data-break="1"' : ''
    return i === 0
...
// paginators must always split it into per-part chunks (not just on overflow).
export function hasForcedPartBreak(b) {
  return b?.type === 'question' && Number(b.partsCols) !== 2 &&
