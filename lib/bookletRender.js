...
function questionHtml(b, ctx) {
  ...
  const partsHtml = hasParts
    ? `<div class="${partsCls}">${b.parts.map((p, i) => {
        ...
        const partAnswer = ctx.solutions
          ...
          : (p.answerObj ? mathObjHtml(p.answerObj) : dottedLines(lineCount(p.lines, 3)))
  ...
  const bottomAnswer = hasParts
    ...
    : (ctx.solutions
        ...
        : (b.answerObj ? mathObjHtml(b.answerObj) : dottedLines(lineCount(b.lines, 6))))
...
function calloutBody(b, { image = true } = {}) {
  ...
  if (b.twoCol) {
    return `<div class="bk-callout-2col"><div>${rich(b.body)}</div><div>${rich(b.body2)}</div></div>${img}${obj}`
  }
...
export function blockHtml(b, ctx) {
  switch (b.type) {
    ...
    case 'steps':
      return calloutBlock('steps', 'Steps', `${stepsHtml(b.body)}${b.mathObj ? mathObjHtml(b.mathObj) : ''}`, 'bk-callout-tint')
    case 'text': {
