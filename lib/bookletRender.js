...
export function questionChunksHtml(b, ctx) {
  ...
  return parts.map((p, i) => {
    ...
    return i === 0
      ? `<div class="bk-block bk-q">${qHeadHtml(b, ctx)}${inner}</div>`
      : `<div class="bk-block bk-q bk-q-cont">${inner}</div>`
  })
}
...
export function blockHtml(b, ctx) {
  switch (b.type) {
    ...
    case 'steps':
      return calloutBlock('steps', 'Steps', `${stepsHtml(b.body)}${b.mathObj ? mathObjHtml(b.mathObj) : ''}${b.blankSpace != null ? blankSpace(b.blankSpace === '' ? 4 : b.blankSpace) : ''}`, 'bk-callout-tint')
    case 'text': {
...
.bk-stim-title{ font-size:16px; font-weight:600; margin:0 0 2px; }
.bk-stim-source{ font-size:13px; font-style:italic; color:#555; margin:0 0 10px; }
