...
export function bookletRenderItems(blocks, { solutions, meta } = {}) {
  ...
  if (hw.length > 0) {
    ...
    const pushBlock = (b) => {
      ...
      items.push({ html: blockHtml(b, ctx), chunks: questionChunksHtml(b, ctx), forceChunks: hasForcedPartBreak(b), homework: true })
    }
    if (isEnglish || /chem/i.test(meta?.subject || '')) {
      ...
      const pushGroup = (list, label, newPage = false) => {
        ...
        items.push({ html: hwHeadingHtml(label), homework: true, pageBreakBefore: newPage })
  ...
  if (quiz.length > 0) {
    ...
    for (const b of quiz) {
      ...
      items.push({ html: blockHtml(b, ctx), chunks: questionChunksHtml(b, ctx), forceChunks: hasForcedPartBreak(b), quiz: true })
    }
  }
