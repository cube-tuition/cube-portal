...
function qPartHtml(p, i, ctx) {
  ...
  return `<div class="bk-part">
    ...
    <div class="bk-part-body">${partMarks}${partObjFloats ? partObj : ''}${rich(p.prompt)}${imageHtml(p.image, 'bk-img', imgOpts(p))}${partObjFloats ? '' : partObj}${embeddedTable(p)}${partAnswer}</div>
  </div>`
}
