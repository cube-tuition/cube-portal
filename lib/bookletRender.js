...
export function newBlock(type) {
  ...
  switch (type) {
    ...
    case 'question': return { ...base, prompt: '', image: '', parts: [], marks: '', solution: '', solutionImage: '' }
...
export function coverHtml(meta = {}, { solutions } = {}) {
  ...
  return `<div class="bk-cv" style="--accent:${accent}">
    ...
    <div class="bk-cv-subject">
      ...
      ${meta.topic ? `<div class="bk-cv-topic">${esc(meta.topic)}</div>` : ''}
    </div>
...
export function bookletRenderItems(blocks, { solutions, meta } = {}) {
  ...
  for (const b of content) {
    ...
    items.push({ html: blockHtml(b, { solutions, qNum: qn }) })
  }
