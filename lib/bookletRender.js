...
export function newBlock(type) {
  ...
  switch (type) {
    ...
    case 'mathobj':  return {
      ...
      // cartesian
      xMin: '-5', xMax: '5', yMin: '-5', yMax: '5', grid: true, points: '', lines: '',
      // number line
...
function cartesianSvg(b) {
  ...
  // Plain upright axis letters in the booklet font (not LaTeX-style italics).
  s += `<text x="${X(xMax) + 16}" y="${Y(ay) + 5}" font-size="14.5" font-family="'Avenir Next','Avenir','Segoe UI',system-ui,sans-serif" fill="#222">x</text>`
  s += `<text x="${X(ax) - 4}" y="${Y(yMax) - 17}" font-size="14.5" font-family="'Avenir Next','Avenir','Segoe UI',system-ui,sans-serif" fill="#222">y</text>`
