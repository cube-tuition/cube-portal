...
import { autoLines } from './answerLines'
import { joinDisplayMath } from './displayMath'
import { qbankImageUrl } from './qbank'

// ── Block factory ─────────────────────────────────────────────────────────────
let _seq = 0
const uid = () => `b_${Date.now().toString(36)}_${(_seq++).toString(36)}`
// Question parts carry their own stable id so a student's typed answer stays
// bound to the right part when the workbook is edited later — inserting a
// part (b) must not shift everyone's answers onto the wrong prompts.
export const partUid = () => `p_${Date.now().toString(36)}_${(_seq++).toString(36)}`
...
export const BLOCK_TYPES = [
  ...
  { type: 'derivation', label: 'Derivation',      icon: '∴', group: 'Boxes' },
  { type: 'steps',      label: 'Steps',           icon: '≡', group: 'Boxes' },
  { type: 'stimulus',   label: 'Stimulus text',   icon: '❝', group: 'Boxes' },
  { type: 'teachernotes', label: "Teacher's Notes", icon: '🖊', group: 'Boxes' },
...
export function newBlock(type) {
  ...
  switch (type) {
    ...
    case 'derivation': return { ...base, title: 'Derivation', body: '', body2: '', twoCol: false, image: '' }
    case 'steps':      return { ...base, title: 'Steps', heading: '', body: '' }
    case 'stimulus': return { ...base, title: '', source: '', body: '', image: '', twoCol: false }
    ...
    case 'mathobj':  return {
      ...
      // dot plot — raw data; the axis range may be widened past the data
      dpTitle: '', dpData: '', dpMin: '', dpMax: '', dpStep: '1', dpXLabel: '',
      // table of values
      tbX: '0, 1, 2, 3', tbY: '', tbXLabel: 'x', tbYLabel: 'y',
      // stem-and-leaf plot — slBack pairs a second (left-hand) data set with the first
      slTitle: '', slData: '', slLeaf: '1', slHeaders: true, slBack: false, slData2: '', slLeft: '', slRight: '',
    }
    ...
    // A space the TEACHER writes into: exemplars, board notes. Printed as blank
    // lines to handwrite on; online it becomes a live box the teacher types in,
    // mirrored read-only on every student's copy.
    case 'teachernotes': return { ...base, title: "Teacher's Notes", lines: 6 }
    case 'table':    return { ...base, headerRow: false, headerCol: false, width: '', align: '', cellAlign: '', colWidths: [], rows: [['', '', ''], ['', '', ''], ['', '', '']] }
...
// Plain-text substitutions (outside $…$ maths):
//   ->  →  →     <-  →  ←     <-> / <=>  →  ⇌ (equilibrium)
//   5*2 → 5 × 2  (a "*" used as a multiplication operator)
//
// The "*" rule only fires BETWEEN two operands — digit/letter/closing bracket on
// the left, digit/letter/opening bracket on the right. That deliberately leaves
// alone the two other things asterisks are used for: a leading footnote marker
// ("*Longest job affordable…"), and the **bold** markers, whose asterisks are
// always adjacent to another asterisk and so never match.
const MATH_SEG = /(\$\$[^$]*\$\$|\$[^$]+\$)/
const TIMES_OP = /(?<=[\w)\]])[ \t]*(?<!\*)\*(?!\*)[ \t]*(?=[\w([])/g
function textOps(s) {
...
function rich(body) {
  ...
  let blankRun = 0    // consecutive empty lines seen
  ...
  for (const raw of lines) {
    ...
    if (bullet) {
      ...
      html += `<li>${boldify(latexToHtml(textOps(bullet[2])))}</li>`
    } else if (line.trim() === '') {
      closeList()
      // One blank line is the normal paragraph gap (unchanged, so existing
      // booklets keep their layout). Each EXTRA consecutive blank line adds a
      // real empty line of space — press Enter more times, get more space.
      blankRun++
      if (blankRun > 1) html += '<div class="bk-blankline"></div>'
    } else {
      blankRun = 0
      closeList()
      ...
      html += `<p class="bk-p${centered ? ' bk-center' : ''}">${boldify(latexToHtml(textOps(content)))}</p>`
    }
  }
  closeList()
  return html
...
function imageHtml(path, cls = 'bk-img', opts = {}) {
  ...
  // "center" keeps the image in the flow (no wrapping) but centres it in the
  // column — the natural choice for a diagram that belongs to a question part.
  const centred = opts.pos === 'center' && !/bk-img-center/.test(cls)
  return `<div class="${cls}${centred ? ' bk-img-center' : ''}"><img src="${esc(url)}" alt=""${hasW ? ` style="width:${w}%"` : ''} /></div>`
}
...
// Online workbooks are typed into, not written on, so their answer space is a
// single empty box rather than ruled lines. The box is sized from the same
// line count the printed copy would have used, so a one-line answer and a
// 22-line extended response still look — and behave — differently.
const ANSWER_LINE_H = 26
const answerBox = (lines) => {
  const n = Number(lines)
  if (!Number.isFinite(n) || n <= 0) return ''
  return `<div class="bk-answer-box" style="min-height:${Math.max(2, n) * ANSWER_LINE_H}px"></div>`
}
// Student-copy answer area for a question or part: maths object, blank space
// or writing space. answerType is authoritative; legacy rows without it are
// inferred from which fields are set. In an online workbook every writing
// space becomes a typing box.
const answerArea = (h, dfltLines, ctx) => {
  const t = h.answerType || (h.answerObj ? 'object' : (h.answerBlank ?? '') !== '' ? 'blank' : 'lines')
  // A maths object (number plane, number line) is drawn on, not typed in — it
  // stays as-is even online.
  if (t === 'object' && h.answerObj) return mathObjHtml(h.answerObj)
  if (t === 'blank') {
    const cm = (h.answerBlank ?? '') === '' ? 4 : h.answerBlank
    return ctx?.online ? answerBox(Math.round((Number(cm) || 4) * 37.8 / ANSWER_LINE_H)) : blankSpace(cm)
  }
  const n = lineCount(h.lines, dfltLines)
  return ctx?.online ? answerBox(n) : dottedLines(n)
}
...
function qPartHtml(p, i, ctx) {
  ...
  // A part carrying options is a multiple-choice sub-question: it shows the
  // option list instead of writing lines, and ticks the answer in the solutions
  // copy. Any solution text still prints underneath as the explanation.
  const isMcqPart = !!(p.options && p.options.length)
  // data-pid/data-opt: inert in print, but the online workbook uses them to
  // let a student click their choice (the slot id matches answerSlots' key).
  const partOpts = isMcqPart
    ? `<div class="bk-opts" data-pid="${esc(p.id || `#${i}`)}">${p.options.map(o => {
        const correct = ctx.solutions && p.answer && o.k === p.answer
        return `<div class="bk-opt ${correct ? 'bk-opt-correct' : ''}" data-opt="${esc(o.k)}"><span class="bk-opt-k">${esc(o.k)}.</span> <span>${boldify(latexToHtml(textOps(o.t)))}</span>${correct ? ' <span class="bk-tick">✓</span>' : ''}</div>`
      }).join('')}</div>`
    : ''
  const partAnswer = isMcqPart
    ? (ctx.solutions && partSol.trim()
        ? `<div class="bk-solbox bk-part-sol"><p class="bk-sol-label">Answer: ${esc(p.answer || '')}</p><div class="bk-sol-body">${partSol}</div></div>`
        : '')
    : ctx.solutions
      ...
      : answerArea(p, autoLines(p, 3), ctx)
  const partObj = p.mathObj ? mathObjHtml(p.mathObj) : ''
  const partObjFloats = !!p.mathObj && (p.mathObj.pos === 'left' || p.mathObj.pos === 'right')
  // Per-part mark allocation — a bare number in the right margin (exam style),
  // shown wherever question marks show.
  const partMarks = (p.marks && !ctx?.hideMarks)
    ? `<span class="bk-part-marks">${esc(p.marks)}</span>` : ''
  return `<div class="bk-part" data-pid="${esc(String(p.id ?? i))}">
    <div class="bk-part-label">${String.fromCharCode(97 + i)}.</div>
    <div class="bk-part-body">${partMarks}${partObjFloats ? partObj : ''}${rich(p.prompt)}${imageHtml(p.image, 'bk-img', imgOpts(p))}${partObjFloats ? '' : partObj}${embeddedTable(p)}${partOpts}${partAnswer}</div>
  </div>`
}

/*
 * A question block's marks.
 *
 * When the parts carry their own marks they are the source of truth: the header
 * total is their sum. A hand-typed total drifts the moment a part is added or
 * removed — a question whose parts read 3, 1 and 2 was printing "(7 marks)".
 * Blocks with no part marks keep their typed value (an MCQ defaults to 1).
 */
export function blockMarks(b) {
  if (!b) return 0
  const parts = Array.isArray(b.parts) ? b.parts : []
  if (parts.length && parts.some((p) => p.marks != null && p.marks !== '')) {
    return parts.reduce((sum, p) => sum + (Number(p.marks) || 0), 0)
  }
  if (b.type === 'mcq') return Number(b.marks) || 1
...
function qHeadHtml(b, ctx) {
  const total = blockMarks(b)
  const marks = (total && !ctx?.hideMarks) ? `<span class="bk-marks">(${total} mark${total === 1 ? '' : 's'})</span>` : ''
...
function qBottomAnswer(b, ctx) {
  ...
  const solContent = `${rich(b.solution)}${imageHtml(b.solutionImage, 'bk-img bk-img-center')}${b.solutionMathObj ? mathObjHtml(b.solutionMathObj) : ''}`
  ...
  return hasParts
    ...
    : (ctx.solutions
        ...
        : answerArea(b, autoLines(b, 6), ctx))
}
...
// chunk 0 = header + its parts; each later chunk = the next run of parts in a
// continuation wrapper (the overall solution box rides on the last chunk).
// Paginators fall back to these chunks so an oversized question breaks between
// parts instead of stretching its page. Returns null when it can't/needn't split.
export function questionChunksHtml(b, ctx) {
  if (!b || b.type !== 'question') return null
  const parts = b.parts || []
  // A break asked for on the FIRST part sends every part to a new page and
  // leaves the question's stem — and any stimulus table or diagram with it —
  // on the page it started on. That is worth supporting even for a one-part
  // question, so the two-part minimum only applies when it is not set.
  const headBreak = !!parts[0]?.pageBreakBefore
  if (parts.length < 2 && !headBreak) return null
  const twoCol = Number(b.partsCols) === 2
  const partsCls = twoCol ? 'bk-parts bk-parts-2col' : 'bk-parts'
  // Where each chunk starts. One column splits between every part, so a long
  // question fills each page as far as it will go. Two columns split only where
  // a part was explicitly asked to start a page — chunking per part there would
  // strand every part alone in a one-item grid and lose the two-up layout.
  const starts = twoCol
    ? [0, ...parts.map((_, i) => i).filter(i => i >= 1 && parts[i].pageBreakBefore)]
    : parts.map((_, i) => i)
  if (starts.length < 2 && !headBreak) return null
  const bottom = qBottomAnswer(b, ctx)
  const bid = b.id ? ` data-bid="${esc(b.id)}"` : ''
  const out = []
  // The stem rides alone on the page the question started on.
  if (headBreak) out.push(`<div${bid} class="bk-block bk-q">${qHeadHtml(b, ctx)}${embeddedTable(b)}</div>`)
  starts.forEach((from, k) => {
    const to = k + 1 < starts.length ? starts[k + 1] : parts.length
    // Part letters keep counting across chunks — (c) stays (c) on the new page.
    const run = parts.slice(from, to).map((p, j) => qPartHtml(p, from + j, ctx)).join('')
    const inner = `<div class="${partsCls}">${run}</div>${to === parts.length ? bottom : ''}`
    // A chunk starting at a part marked "start on a new page" is tagged so the
    // paginators force a page break before it.
    const brk = parts[from]?.pageBreakBefore ? ' data-break="1"' : ''
    out.push(from === 0 && !headBreak
      ? `<div${bid} class="bk-block bk-q">${qHeadHtml(b, ctx)}${embeddedTable(b)}${inner}</div>`
      : `<div${bid}${brk} class="bk-block bk-q bk-q-cont">${inner}</div>`)
  })
  return out
}

// True when a question asks to break before one of its parts, so the paginators
// must always split it into chunks (not just on overflow). A break on the first
// part counts: it splits the stem away from the parts.
export function hasForcedPartBreak(b) {
  return b?.type === 'question' && (b.parts || []).some(p => !!p.pageBreakBefore)
}
...
function stimulusBodyHtml(body) {
  ...
  const flush = () => {
    ...
    out.push(`<p class="bk-stim-stanza">${stanza.join('<br>')}</p>`)
    stanza = []
  }
  for (const line of text.split('\n')) {
    if (!line.trim()) {
      // One blank line ends the stanza (the gap it already had); each further
      // blank line adds a real empty line, as it does in rich().
      blankRun++
      flush()
      if (blankRun > 1) out.push('<div class="bk-blankline"></div>')
      continue
    }
    blankRun = 0
    stanza.push(boldify(latexToHtml(line)))
  }
  flush()
  return out.join('')
}

// A stimulus image honours the same width/position controls as every other
// block's image. It used to ignore them, so a full-page scan could only render
// at 100% and take a whole page on its own.
function stimulusHtml(b) {
  ...
  return `<div class="bk-block bk-stimulus">
    ...
    <div class="bk-stim-body${b.twoCol ? ' bk-stim-2col' : ''}">${stimulusBodyHtml(b.body)}</div>
    ${imageHtml(b.image, 'bk-img bk-img-center', imgOpts(b))}
  </div>`
}
...
function mcqHtml(b, ctx) {
  ...
  return `<div class="bk-block bk-q">
    ...
    ${qObjFloats ? qObj : ''}
    ${b.prompt ? `<div class="bk-q-prompt">${rich(b.prompt)}</div>` : ''}
    ${imageHtml(b.image, 'bk-img', imgOpts(b))}
    ${qObjFloats ? '' : qObj}
    ${embeddedTable(b)}
    <div class="bk-opts" data-pid="">${opts}</div>
    ${expl}
  </div>`
}
...
function calloutIcon(type) {
  ...
  const inner = {
    ...
    // One line following from another: a statement, an arrow down, the result.
    derivation: `<path d="M5 6.5h14M5 18.5h14M12 9.5v5.5" ${a}/><path d="M9.6 12.6L12 15.2l2.4-2.6" ${a}/>`,
...
function calloutBody(b, { image = true } = {}) {
  ...
  const tbl = embeddedTable(b)
  if (b.twoCol) {
    return `<div class="bk-callout-2col"><div>${rich(b.body)}</div><div>${rich(b.body2)}</div></div>${img}${obj}${tbl}${blank}`
  }
  ...
  const { html: bodyHtml, trailing } = richWithTable(b.body, tbl)
  return `${pre}${bodyHtml}${post}${trailing}${blank}`
}
...
function cartesianSvg(b) {
  ...
  // Intercept coordinate labels carry a white halo (paint-order stroke) and sit
  // clear of the axes so they stay readable over curves and gridlines. They can
  // be suppressed with interceptLabels:false (keeps the axis numbers) — useful
  // for "find the equation from the graph" questions where the coordinates
  // would give away the answer.
  const showIntLabels = b.interceptLabels !== false
  const xDot = (xi) => { if (!showIntLabels) return; s += `<circle cx="${X(xi)}" cy="${Y(0)}" r="3.4" fill="#0F766E"/><text x="${X(xi) + 8}" y="${Y(0) + 18}" font-size="11" font-weight="bold" paint-order="stroke" stroke="#fff" stroke-width="3.5" stroke-linejoin="round" fill="#0F766E">(${fmtN(xi)}, 0)</text>` }
  const yDot = (yi) => { if (!showIntLabels) return; s += `<circle cx="${X(0)}" cy="${Y(yi)}" r="3.4" fill="#0F766E"/><text x="${X(0) + 9}" y="${Y(yi) - 8}" font-size="11" font-weight="bold" paint-order="stroke" stroke="#fff" stroke-width="3.5" stroke-linejoin="round" fill="#0F766E">(0, ${fmtN(yi)})</text>` }
...
function xyTableHtml(o) {
  ...
  const valCell = (v) => `<td>${boldify(latexToHtml(textOps(v ?? '')))}</td>`
...
function stemLeafHtml(o) {
  const parse = (v) => String(v ?? '').split(/[\s,]+/).map(t => t.trim()).filter(Boolean)
    .map(Number).filter(Number.isFinite)
  const p = Number(o.slLeaf) > 0 ? Number(o.slLeaf) : 1     // leaf place value (1 = units)
  // Back-to-back: a second data set whose leaves run LEFT from the shared stems.
  const back = !!o.slBack
  const rightNums = parse(o.slData), leftNums = back ? parse(o.slData2) : []
  if (!rightNums.length && !leftNums.length) return ''
  const scale = (nums) => nums.map(v => Math.round(v / p)).sort((a, b) => a - b)   // values in leaf-units
  const R = scale(rightNums), Lf = scale(leftNums)
  const stemOf = (s) => Math.floor(s / 10)
  const leafOf = (s) => ((s % 10) + 10) % 10
  const all = [...R, ...Lf]
  const minStem = stemOf(Math.min(...all)), maxStem = stemOf(Math.max(...all))
  const rows = []
  for (let st = minStem; st <= maxStem; st++) {
    rows.push({
      stem: st,
      r: R.filter(s => stemOf(s) === st).map(leafOf),
      // Left leaves increase outwards from the stem, so they print in reverse.
      l: Lf.filter(s => stemOf(s) === st).map(leafOf).reverse(),
    })
  }
  const dp = p < 1 ? (String(p).split('.')[1] || '').length : 0
  const first = rows.find(r => r.r.length) || rows.find(r => r.l.length) || rows[0]
  const keyStem = first.stem, keyLeaf = first.r[0] ?? first.l[first.l.length - 1] ?? 0
  const keyVal = ((keyStem * 10 + keyLeaf) * p).toFixed(dp)
  const title = String(o.slTitle || '').trim()
  const head = o.slHeaders === false ? '' : back
    ? `<tr><th>${esc(String(o.slLeft || 'Leaf').trim() || 'Leaf')}</th><th>Stem</th><th>${esc(String(o.slRight || 'Leaf').trim() || 'Leaf')}</th></tr>`
    : `<tr><th>Stem</th><th>Leaf</th></tr>`
  const body = rows.map(r => back
    ? `<tr><td class="bk-slp-leaf bk-slp-leaf-l">${r.l.join(' ')}</td><td class="bk-slp-stem">${r.stem}</td><td class="bk-slp-leaf">${r.r.join(' ')}</td></tr>`
    : `<tr><td class="bk-slp-stem">${r.stem}</td><td class="bk-slp-leaf">${r.r.join(' ')}</td></tr>`
  ).join('')
  return `${title ? `<div class="bk-slp-title">${esc(title)}</div>` : ''}` +
    `<table class="bk-slp${back ? ' bk-slp-b2b' : ''}">${head}${body}</table>` +
    `<div class="bk-slp-key">Key: ${keyStem} | ${keyLeaf} = ${keyVal}</div>`
}

// Dot plot — one dot per data value, stacked above its place on the axis. Input
// is the raw data (frequencies are counted here), so a value inside the range
// that never occurs correctly shows an empty column, which is usually the point
// of the question.
function dotplotSvg(b) {
  const vals = String(b.dpData ?? '').split(/[\s,]+/).map(t => parseFloat(t)).filter(Number.isFinite)
  if (!vals.length) return ''
  const step = Math.abs(numOr(b.dpStep, 1)) || 1
  const dMin = Math.min(...vals), dMax = Math.max(...vals)
  // An explicit range may only WIDEN the axis — never narrow it, or data would
  // silently vanish from the plot.
  const uMin = parseFloat(b.dpMin), uMax = parseFloat(b.dpMax)
  const min = Number.isFinite(uMin) ? Math.min(uMin, dMin) : dMin
  let max = Number.isFinite(uMax) ? Math.max(uMax, dMax) : dMax
  if (max <= min) max = min + step
  const units = (max - min) / step
  // The canvas width is FIXED so that every dot plot scales by the same amount
  // inside its container — otherwise a short axis is blown up (giant dots) and a
  // long one shrunk (unreadable numbers). Tick spacing adapts within it, and a
  // plot that doesn't need the full width is centred.
  const W = 520, PAD = 40
  const U = Math.min(70, (W - PAD * 2) / Math.max(units, 1))
  const used = U * units
  const x0 = (W - used) / 2
  const R = 4.6, GAP = 12.4          // dot radius and vertical pitch
  const counts = new Map()           // keyed by exact value, so off-tick data still plots
  for (const v of vals) counts.set(v, (counts.get(v) || 0) + 1)
  const fMax = Math.max(...counts.values())
  const title = String(b.dpTitle || '').trim()
  const xLab = String(b.dpXLabel || '').trim()
  const TH = title ? 24 : 0
  const axisY = TH + 12 + fMax * GAP
  const H = axisY + 28 + (xLab ? 20 : 0)
  const X = (v) => x0 + ((v - min) / step) * U
  let s = ''
  if (title) s += `<text x="${W / 2}" y="16" font-size="13.5" font-weight="bold" text-anchor="middle" fill="#1c1c1c">${esc(title)}</text>`
  s += `<line x1="${x0 - 14}" y1="${axisY}" x2="${x0 + used + 14}" y2="${axisY}" stroke="#222" stroke-width="1.5"/>`
  const dp = String(step).includes('.') ? String(step).split('.')[1].length : 0
  for (let i = 0; i <= Math.round(units) + 1e-9; i++) {
    const v = min + i * step
    if (v > max + 1e-9) break
    s += `<line x1="${X(v)}" y1="${axisY}" x2="${X(v)}" y2="${axisY + 6}" stroke="#222" stroke-width="1.2"/>`
    s += `<text x="${X(v)}" y="${axisY + 21}" font-size="11" text-anchor="middle" fill="#555">${v.toFixed(dp)}</text>`
  }
  for (const [v, c] of [...counts.entries()].sort((p, q) => p[0] - q[0])) {
    for (let k = 0; k < c; k++) {
      s += `<circle cx="${X(v)}" cy="${axisY - 9 - k * GAP}" r="${R}" fill="#B23A3A"/>`
    }
  }
  if (xLab) s += `<text x="${W / 2}" y="${H - 5}" font-size="12.5" text-anchor="middle" fill="#1c1c1c">${esc(xLab)}</text>`
  return `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" width="100%" style="height:auto">${s}</svg>`
}

// Exported so the exam renderer can draw the same figures from a question's
// math_obj without taking on the booklet's stylesheet: this returns the SVG
// itself, and each caller wraps it however its own page is laid out.
export function mathObjectSvg(b) {
  ...
  if (b.objType === 'dotplot') return dotplotSvg(b)
...
function stepsHtml(body) {
  // Each line is a numbered step. Blank lines are not steps, but — as in rich() —
  // every blank line BEYOND the first opens up real space before the next step,
  // so pressing Enter a few times spaces the list out. The gap goes on the
  // following <li> rather than a spacer element, which would take a number.
  const out = []
  let blankRun = 0
  for (const raw of String(body ?? '').split('\n')) {
    const line = raw.trim()
    if (!line) { blankRun++; continue }
    const gap = blankRun > 1 ? ` style="margin-top:${((blankRun - 1) * 1.55).toFixed(2)}em"` : ''
    blankRun = 0
    out.push(`<li${gap}>${boldify(latexToHtml(textOps(line.replace(/^\s*\d+[.)]\s*/, ''))))}</li>`)
  }
  if (!out.length) return ''
  return `<ol class="bk-ol">${out.join('')}</ol>`
}
...
function blockBodyHtml(b, ctx) {
  switch (b.type) {
    ...
    // A proof of a formula the students are about to use. White like a worked
    // solution — it is working, not a result to memorise from a tinted box.
    case 'derivation':
      return calloutBlock('derivation', 'Derivation', calloutBody(b), 'bk-callout-white')
    case 'steps': {
    ...
    case 'text': {
      ...
      const { html: tBody, trailing: tTbl } = richWithTable(b.body, embeddedTable(b))
      return `<div class="bk-block bk-text">${imgFloats(b) ? tImg : ''}${tBody}${imgFloats(b) ? '' : tImg}${tTbl}</div>`
    }
    case 'mathobj':
    ...
    case 'writing': {
      const n = b.lines || 8
      return `<div class="bk-block bk-writing">${b.title ? `<p class="bk-writing-title">${esc(b.title)}</p>` : ''}${ctx?.online ? answerBox(n) : dottedLines(n)}</div>`
    }
    case 'teachernotes': {
      const n = lineCount(b.lines, 6)
      return `<div class="bk-block bk-tnotes"><p class="bk-tnotes-title"><span style="margin-right:5px">🖊</span>${esc(b.title || "Teacher's Notes")}</p>${ctx?.online ? answerBox(n) : dottedLines(n)}</div>`
    }
    case 'table':     return tableHtml(b, false, ctx)
...
function tableHtml(b, embedded = false, ctx = null) {
  const rows = Array.isArray(b.rows) ? b.rows : []
  if (!rows.length) return ''
  const cellHtml = (c) => String(c || '').split('\n').map((line) => boldify(latexToHtml(textOps(line)))).join('<br>')
  // A table can have a header row (across the top), a header column (down the
  // left), or both — the corner cell of a both-headers table belongs to neither
  // axis, so it carries no scope.
  const headerCol = !!b.headerCol
  const cell = (c, tag, scope) => `<${tag}${scope ? ` scope="${scope}"` : ''}>${cellHtml(c)}</${tag}>`
  const headRow = (cells) => `<tr>${cells.map((c, i) =>
    cell(c, 'th', headerCol && i === 0 ? '' : 'col')).join('')}</tr>`
  // Online workbooks: a BLANK body cell is an answer space — the student types
  // straight into it. The renderer only tags it (data-tcell = "row-col" in the
  // block's own row indexing); WorkbookDoc wires up editing, state and saves,
  // exactly like the MCQ data-opt tags. Header cells never become fillable,
  // and print output is untouched (the tag carries no print styling).
  const fillable = !!ctx?.online && !embedded
  const bodyRow = (cells, r0) => `<tr>${cells.map((c, i) => {
    if (headerCol && i === 0) return cell(c, 'th', 'row')
    if (fillable && !String(c ?? '').trim()) return `<td data-tcell="${r0}-${i}"></td>`
    return cell(c, 'td')
  }).join('')}</tr>`
  const head = b.headerRow ? headRow(rows[0]) : ''
  const off = b.headerRow ? 1 : 0
  const body = (b.headerRow ? rows.slice(1) : rows).map((cells, r) => bodyRow(cells, r + off)).join('')
  const w = Number(b.width)
  const hasW = Number.isFinite(w) && w > 0 && w < 100
  const margin = b.align === 'left' ? '0 auto 0 0' : b.align === 'right' ? '0 0 0 auto' : '0 auto'
  // Cells are centred by default, which suits a table of short values and
  // reads badly for one full of sentences. cellAlign:'left' opts a table out.
  const cellCls = b.cellAlign === 'left' ? ' bk-tbl-left' : ''
  ...
  // Embedded tables (inside a callout/question) use a lighter wrapper so they
  // don't inherit the big standalone-block margin.
  const wrapCls = embedded ? 'bk-tbl-embed' : 'bk-block bk-tbl-wrap'
  return `<div class="${wrapCls}"><table class="bk-tbl${cellCls}"${style}>${colgroup}${head}${body}</table></div>`
}

// An optional table embedded in another block (definition, question, part…).
// b.table has the same shape as a standalone table block.
const embeddedTable = (b) => (b && b.table) ? tableHtml(b.table, true) : ''

// A table normally follows the text of the block it is attached to. A body can
// instead place it mid-flow — data the working below refers to belongs above
// that working, not under it — by putting "[[table]]" on a line of its own.
// The marker is always removed, so a body that lost its table never shows it.
const TABLE_MARK = /^[ \t]*\[\[table\]\][ \t]*$/m
function richWithTable(body, tbl) {
  const text = String(body ?? '')
  const at = text.search(TABLE_MARK)
  if (at < 0) return { html: rich(text), trailing: tbl }
  const after = text.slice(at).replace(/^[ \t]*\[\[table\]\][ \t]*\n?/, '')
  // A trailing table can sit tight against the block's bottom edge; one with
  // text under it needs room beneath, so it carries its own spacing class.
  const mid = tbl ? `<div class="bk-tbl-mid">${tbl}</div>` : ''
  return { html: rich(text.slice(0, at)) + mid + rich(after), trailing: '' }
}
...
// The booklet's display name — the title shown on the cover, and reused for the
// Homework / Revision Quiz labels so they always match the cover. Prefers the
// booklet name (e.g. "Linear Relationships 2") over the canonical topic, and
// spells out Chemistry's compact "M8L1" names (legacy "W" names still parse).
function bookletDisplayName(meta = {}) {
  let name = meta.name || meta.topic || ''
  const chemWk = /^M(\d+)[LW](\d+)$/i.exec(name)
  if (chemWk && /chem/i.test(meta.subject || '')) name = `Module ${chemWk[1]} Lesson ${chemWk[2]}`
  return name
}

// `tagline` is a quiet line under the title — the worksheet cover writes
// "Additional Questions" there. The Solutions pill behaves as on any cover.
export function coverHtml(meta = {}, { solutions, tagline } = {}) {
  ...
  const workbookName = bookletDisplayName(meta)   // shown under the year
  ...
  return `<div class="bk-cv" style="--accent:${accent}">
    ...
    <div class="bk-cv-subject">
      ...
      ${tagline ? `<div class="bk-cv-tag">${esc(tagline)}</div>` : ''}
    </div>
...
export function testTotalMarks(blocks = []) {
  return (blocks || []).reduce(
    (sum, b) => (b && (b.type === 'question' || b.type === 'mcq') ? sum + blockMarks(b) : sum), 0)
}
...
export const HW_BLOCK_TYPES = BLOCK_TYPES.filter(t => t.group === 'Questions' || t.type === 'writing' || t.type === 'stimulus' || t.type === 'pagebreak')
...
function homeworkTitleHtml(meta = {}) {
  ...
  const label = bookletDisplayName(meta)
  return `<div class="bk-block bk-section bk-section-center bk-section-hw"><span class="bk-section-title">Homework</span>${label ? `<span class="bk-hw-topic">${esc(label)}</span>` : ''}</div>`
}
...
function revisionQuizTitleHtml(meta = {}, total = 0) {
  return `<div class="bk-block bk-quiz-head">
    ...
    <div class="bk-section bk-section-quiz">
      <div class="bk-quiz-titlewrap"><span class="bk-section-title">Revision Quiz</span>${bookletDisplayName(meta) ? `<span class="bk-hw-topic">${esc(bookletDisplayName(meta))}</span>` : ''}</div>
...
// (parts win over a typed total), defaulting to 1 when blank. Writing space
// contributes nothing.
function quizTotalMarks(quiz) {
  return quiz.reduce((sum, b) => {
    if (b.type !== 'question' && b.type !== 'mcq') return sum
    // A question with neither part marks nor a typed total still counts for 1.
    return sum + (blockMarks(b) || 1)
  }, 0)
}
...
export function bookletRenderItems(blocks, { solutions, meta } = {}) {
  ...
  // Online workbooks are typed into in the portal, so every writing space
  // renders as an empty box instead of ruled lines — in the live preview and
  // in the PDF preview alike, so what the teacher checks is what students get.
  const online = meta?.delivery === 'online'
  ...
  for (const b of content) {
    ...
    const ctx = { solutions, qNum: qn, hideSectionSyllabus, hideMarks: isMaths, online }
    // Section headers always begin on a fresh page — except the very first
    // block, which already sits at the top of the first content page.
    const startNewPage = pendingBreak || (b.type === 'section' && items.length > 0)
    items.push({ html: blockHtml(b, ctx), chunks: questionChunksHtml(b, ctx), forceChunks: hasForcedPartBreak(b), pageBreakBefore: startNewPage })
    pendingBreak = false
  }
  ...
  if (hw.length > 0) {
    ...
    let hq = 0
    let pendingHwBreak = false
    const pushBlock = (b) => {
      // A "New page" block pushes whatever follows onto a fresh page.
      if (b.type === 'pagebreak') { pendingHwBreak = true; return }
      ...
      const ctx = { solutions, qNum: hq, hideMarks: isMaths, online }
      items.push({ html: blockHtml(b, ctx), chunks: questionChunksHtml(b, ctx), forceChunks: hasForcedPartBreak(b), homework: true, pageBreakBefore: pendingHwBreak })
      pendingHwBreak = false
    }
    if (isEnglish || /chem/i.test(meta?.subject || '')) {
      // English and Chemistry homework are a single flexible list: no fixed
      // Foundational / Developmental split. Any subheadings the teacher adds
      // (subtopic blocks) render inline in document order.
      for (const b of hw) pushBlock(b)
    } else {
      const pushGroup = (list, label, newPage = false) => {
        // A group holding nothing but page breaks gets no heading.
        if (!list.some(b => b.type !== 'pagebreak')) return
        items.push({ html: hwHeadingHtml(label), homework: true, pageBreakBefore: newPage || pendingHwBreak })
        pendingHwBreak = false
        for (const b of list) pushBlock(b)
      }
      pushGroup(found, 'MCQ + Foundational Questions')
      // The Developmental Questions subheading always starts on a fresh page.
      pushGroup(dev, 'Developmental Questions', found.length > 0)
    }
  }
  ...
  if (quiz.length > 0) {
    ...
    let zq = 0
    let pendingQuizBreak = false
    for (const b of quiz) {
      // A "New page" block pushes whatever follows onto a fresh page.
      if (b.type === 'pagebreak') { pendingQuizBreak = true; continue }
      if (b.type === 'stimulus') zq = 0
      if (b.type === 'question' || b.type === 'mcq') zq++
      const ctx = { solutions, qNum: zq, online }
      items.push({ html: blockHtml(b, ctx), chunks: questionChunksHtml(b, ctx), forceChunks: hasForcedPartBreak(b), quiz: true, pageBreakBefore: pendingQuizBreak })
      pendingQuizBreak = false
    }
  }
...
/* A page is exactly A4 (794 x 1123 @ 96dpi) — a FIXED height, not a minimum.
   With min-height a block taller than the page stretched it, and the PDF then
   captured an over-tall page and sliced it, cutting the footer. Blocks that
   don't fit are split across pages by lib/paginate.js instead. */
.bk-page{ position:relative; width:794px; height:1123px; background:#fff; padding:38px 48px 60px; box-sizing:border-box; overflow:hidden; }
...
.bk-cv-tag{ font-weight:500; font-size:13px; letter-spacing:3px; text-transform:uppercase; color:rgba(54,68,102,.45); margin-top:14px; }
...
.bk-quiz-name{ font-size:13px; font-weight:600; color:#1f1f1f; margin:0 0 4px 0; padding-left:10px; }
...
/* One empty line of vertical space — emitted for each consecutive blank line
   beyond the first in a rich-text body (multiple Enters = growing gap). */
.bk-blankline{ height:1.55em; font-size:16px; }
...
.bk-slp td.bk-slp-leaf-l{ text-align:right; border-right:1.6px solid #333; }
.bk-slp-b2b td.bk-slp-stem{ text-align:center; padding:2.5px 12px; }
...
.bk-marks{ font-size:16px; font-weight:600; color:#555; }
.bk-part-marks{ float:right; margin-left:16px; font-size:16px; font-weight:600; color:#1c1c1c; }
...
/* Two-column stimulus (long poems): stanzas flow across two columns and are
   kept whole so a verse never splits between columns. */
.bk-stim-2col{ column-count:2; column-gap:34px; }
.bk-stim-2col .bk-stim-stanza{ break-inside:avoid; -webkit-column-break-inside:avoid; }
...
.bk-solbox{ border:1px solid #c9c9c9; padding:12px 16px; margin-top:12px; overflow-wrap:break-word; word-break:break-word; }
.bk-sol-label{ font-style:italic; font-size:14px; color:#333; margin:0 0 6px; overflow-wrap:break-word; word-break:break-word; }
.bk-sol-body{ font-size:16px; overflow-wrap:break-word; word-break:break-word; }
.bk-muted{ color:#aaa; }
.bk-lines{ margin-top:12px; }
/* Online workbooks: an empty box to type into, in place of ruled lines. */
.bk-answer-box{ margin-top:12px; border:1px solid #c3cee6; border-radius:8px; background:#fcfdff; }
.bk-tnotes-title{ margin:0 0 2px; font-size:11.5px; font-weight:600; letter-spacing:.05em; text-transform:uppercase; color:#0E7A5F; font-family:'Avenir Next','Avenir','Segoe UI',system-ui,sans-serif; }
...
.bk-tbl-embed{ margin:12px 0 4px; }
.bk-tbl-mid .bk-tbl-embed{ margin:6px 0 16px; }
...
/* A table of sentences rather than values: cells read from the left. The
   header row stays centred, so the table still reads as a table. */
.bk-tbl-left td{ text-align:left; }
/* Footer */
