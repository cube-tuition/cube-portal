...
export function newBlock(type) {
  ...
  switch (type) {
    ...
    case 'mathobj':  return {
      ...base, objType: 'cartesian', width: '60',
      // cartesian
...
const parseMCLines = (v) => {
  const rows = Array.isArray(v)
    ? v.map(l => ({ m: Number(l.m), c: Number(l.c), label: String(l.label ?? '').trim() }))
    : String(v ?? '').split('\n').map(l => l.trim()).filter(Boolean).map(l => {
        const parts = l.split(',').map(p => p.trim())
        return { m: Number(parts[0]), c: Number(parts[1]), label: parts.slice(2).join(', ') }
      })
  return rows.filter(l => Number.isFinite(l.m) && Number.isFinite(l.c))
}
...
export function blockHtml(b, ctx) {
  switch (b.type) {
    ...
    case 'mathobj': {
      ...
      return `<div class="bk-block bk-mathobj"><div class="bk-mathobj-inner"${style}>${mathObjectSvg(b)}</div></div>`
    }
    case 'image': {
