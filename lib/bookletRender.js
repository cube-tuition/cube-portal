...
function qPartHtml(p, i, ctx) {
  ...
  const partOpts = isMcqPart
    ? `<div class="bk-opts">${p.options.map(o => {
        const correct = ctx.solutions && p.answer && o.k === p.answer
        return `<div class="bk-opt ${correct ? 'bk-opt-correct' : ''}"><span class="bk-opt-k">${esc(o.k)}.</span> <span>${boldify(latexToHtml(textOps(o.t)))}</span>${correct ? ' <span class="bk-tick">✓</span>' : ''}</div>`
      }).join('')}</div>`
    : ''
