...
export function newBlock(type) {
  ...
  switch (type) {
    ...
    case 'mathobj':  return {
      ...base, objType: 'cartesian', width: '60', caption: '',
      // cartesian
      xMin: '-5', xMax: '5', yMin: '-5', yMax: '5', grid: true, intercepts: false, points: '', lines: '',
      // number line
      nlMin: '0', nlMax: '10', nlStep: '1', nlPoints: '',
      // box plot
      bpMin: '', bpQ1: '', bpMed: '', bpQ3: '', bpMax: '',
    }
...
// One entry per line: "x, y[, label]"
const parseXYPoints = (t) => String(t ?? '').split('\n').map(l => l.trim()).filter(Boolean).map(l => {
  const parts = l.replace(/[()]/g, '').split(',').map(p => p.trim())
  return { x: Number(parts[0]), y: Number(parts[1]), label: parts.slice(2).join(', ') }
}).filter(p => Number.isFinite(p.x) && Number.isFinite(p.y))
// One entry per line: "m, c[, label]" → the line y = mx + c
const parseMCLines = (t) => String(t ?? '').split('\n').map(l => l.trim()).filter(Boolean).map(l => {
  const parts = l.split(',').map(p => p.trim())
  return { m: Number(parts[0]), c: Number(parts[1]), label: parts.slice(2).join(', ') }
}).filter(p => Number.isFinite(p.m) && Number.isFinite(p.c))

function cartesianSvg(b) {
  let xMin = Math.round(numOr(b.xMin, -5)), xMax = Math.round(numOr(b.xMax, 5))
  let yMin = Math.round(numOr(b.yMin, -5)), yMax = Math.round(numOr(b.yMax, 5))
  ...
  let s = ''
  if (b.grid !== false) {
    for (let x = xMin; x <= xMax; x++) s += `<line x1="${X(x)}" y1="${Y(yMax)}" x2="${X(x)}" y2="${Y(yMin)}" stroke="#c9c9c9" stroke-width="0.6"/>`
    for (let y = yMin; y <= yMax; y++) s += `<line x1="${X(xMin)}" y1="${Y(y)}" x2="${X(xMax)}" y2="${Y(y)}" stroke="#c9c9c9" stroke-width="0.6"/>`
  }
  ...
  s += `<text x="${X(ax) - 4}" y="${Y(yMax) - 17}" font-size="15" ${axisFont} fill="#222">y</text>`
  const xStep = (xMax - xMin) > 16 ? 2 : 1
  const yStep = (yMax - yMin) > 16 ? 2 : 1
  for (let x = xMin; x <= xMax; x += xStep) if (x !== 0) s += `<text x="${X(x)}" y="${Y(ay) + 14}" font-size="10.5" text-anchor="middle" fill="#555">${x}</text>`
  for (let y = yMin; y <= yMax; y += yStep) if (y !== 0) s += `<text x="${X(ax) - 6}" y="${Y(y) + 3.5}" font-size="10.5" text-anchor="end" fill="#555">${y}</text>`
  if (xMin <= 0 && 0 <= xMax && yMin <= 0 && 0 <= yMax) s += `<text x="${X(0) - 6}" y="${Y(0) + 14}" font-size="10.5" text-anchor="end" fill="#555">O</text>`
  // Rules y = mx + c, clipped to the visible box.
  for (const ln of parseMCLines(b.lines)) {
    const cand = [[xMin, ln.m * xMin + ln.c], [xMax, ln.m * xMax + ln.c]]
    if (ln.m !== 0) cand.push([(yMin - ln.c) / ln.m, yMin], [(yMax - ln.c) / ln.m, yMax])
    const inside = cand.filter(([x, y]) => x >= xMin - 1e-9 && x <= xMax + 1e-9 && y >= yMin - 1e-9 && y <= yMax + 1e-9)
    if (inside.length < 2) continue
    inside.sort((a, b2) => a[0] - b2[0])
    const [x1, y1] = inside[0], [x2, y2] = inside[inside.length - 1]
    s += `<line x1="${X(x1)}" y1="${Y(y1)}" x2="${X(x2)}" y2="${Y(y2)}" stroke="#325099" stroke-width="1.8"/>`
    if (ln.label) s += `<text x="${X(x2) - 4}" y="${Y(y2) - 7}" font-size="11.5" font-weight="bold" fill="#325099" text-anchor="end">${esc(ln.label)}</text>`
    // Optional intercept labels: mark where the line crosses each axis.
    if (b.intercepts) {
      const fmtN = (v) => { const r = Math.round(v * 100) / 100; return String(r) }
      if (0 >= xMin && 0 <= xMax && ln.c >= yMin && ln.c <= yMax) {
        s += `<circle cx="${X(0)}" cy="${Y(ln.c)}" r="3.4" fill="#0F766E"/>`
        s += `<text x="${X(0) + 7}" y="${Y(ln.c) - 6}" font-size="11" font-weight="bold" fill="#0F766E">(0, ${fmtN(ln.c)})</text>`
      }
      if (ln.m !== 0) {
        const xi = -ln.c / ln.m
        if (xi >= xMin && xi <= xMax && 0 >= yMin && 0 <= yMax && Math.abs(xi) > 1e-9) {
          s += `<circle cx="${X(xi)}" cy="${Y(0)}" r="3.4" fill="#0F766E"/>`
          s += `<text x="${X(xi) + 7}" y="${Y(0) + 16}" font-size="11" font-weight="bold" fill="#0F766E">(${fmtN(xi)}, 0)</text>`
        }
      }
    }
  }
  for (const p of parseXYPoints(b.points)) {
    ...
    if (p.label) s += `<text x="${X(p.x) + 6}" y="${Y(p.y) - 6}" font-size="11.5" font-weight="bold" fill="#B23A3A">${esc(p.label)}</text>`
  }
...
function boxplotSvg(b) {
  const v = [numOr(b.bpMin, 0), numOr(b.bpQ1, 2), numOr(b.bpMed, 5), numOr(b.bpQ3, 7), numOr(b.bpMax, 10)].sort((a, c) => a - c)
  ...
  const aMin = Math.floor((mn - span * 0.08) / step) * step
  const aMax = Math.ceil((mx + span * 0.08) / step) * step
  const PAD = 44, W = 520, H = 130
  const X = (x) => PAD + ((x - aMin) / (aMax - aMin)) * (W - PAD * 2)
  const boxTop = 26, boxBot = 66, mid = (boxTop + boxBot) / 2, Ly = 96
  let s = ''
  // whiskers
  ...
  // box + median
  s += `<rect x="${X(q1)}" y="${boxTop}" width="${X(q3) - X(q1)}" height="${boxBot - boxTop}" fill="#EEF4FF" stroke="#222" stroke-width="1.4"/>`
  s += `<line x1="${X(med)}" y1="${boxTop}" x2="${X(med)}" y2="${boxBot}" stroke="#222" stroke-width="1.8"/>`
  // axis
  s += `<line x1="${PAD - 14}" y1="${Ly}" x2="${W - PAD + 14}" y2="${Ly}" stroke="#222" stroke-width="1.3"/>`
  s += `<path d="M ${W - PAD + 14} ${Ly} l -8 -4 v 8 z" fill="#222"/>`
...
export function blockHtml(b, ctx) {
  switch (b.type) {
    ...
    case 'mathobj': {
      ...
      const style = Number.isFinite(w) && w > 0 && w <= 100 ? ` style="width:${w}%"` : ' style="width:60%"'
      const cap = (b.caption || '').trim() ? `<p class="bk-img-caption">${boldify(latexToHtml(b.caption))}</p>` : ''
      return `<div class="bk-block bk-mathobj"><div class="bk-mathobj-inner"${style}>${mathObjectSvg(b)}${cap}</div></div>`
    }
    case 'image': {
