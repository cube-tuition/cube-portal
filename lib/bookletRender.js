...
export function newBlock(type) {
  ...
  switch (type) {
    case 'section':  return { ...base, number: '', title: 'Section title', syllabus: '' }
...
// two-column split with a thin divider. Any image is placed full-width beneath.
function calloutBody(b, { image = true } = {}) {
  const img = image ? imageHtml(b.image, 'bk-img bk-img-center') : ''
  if (b.twoCol) {
    return `<div class="bk-callout-2col"><div>${rich(b.body)}</div><div>${rich(b.body2)}</div></div>${img}`
  }
  return `${rich(b.body)}${img}`
}
...
export function blockHtml(b, ctx) {
  switch (b.type) {
    case 'section': {
      // Optional syllabus dot-points band underneath the header (Chemistry):
      // a lighter sub-band carrying the syllabus outcomes for the section.
      const syllabus = (b.syllabus && b.syllabus.trim())
    ...
    case 'formula':
      return calloutBlock('formula', b.title || 'Formula', calloutBody(b), 'bk-callout-tint')
    case 'note':
      return calloutBlock('note', b.title || 'Note', calloutBody(b, { image: false }), 'bk-callout-clear')
    case 'definition':
      return calloutBlock('definition', b.title || 'Definition', calloutBody(b), 'bk-callout-tint')
    case 'worked':
      return calloutBlock('worked', b.title || 'Worked Solution', calloutBody(b), 'bk-callout-white')
    case 'steps':
...
// Content-only render items, each tagged with its source block id and (when the
// item immediately follows a manual "New page") that pagebreak block's id. Used
// by the builder to measure where physical pages actually fall, so its per-page
// headers mirror the printed booklet (manual breaks AND automatic overflow).
export function contentRenderItems(blocks, { solutions } = {}) {
  const content = (blocks || []).filter(b => sectionOf(b) === 'content')
  const items = []
  let qn = 0
  let pendingBreak = false
  let pendingBreakId = null
  for (const b of content) {
    if (b.type === 'pagebreak') { pendingBreak = true; pendingBreakId = b.id; continue }
    if (b.type === 'question' || b.type === 'mcq') qn++
    items.push({ id: b.id, html: blockHtml(b, { solutions, qNum: qn }), pageBreakBefore: pendingBreak, breakId: pendingBreakId })
    pendingBreak = false
    pendingBreakId = null
  }
  return items
