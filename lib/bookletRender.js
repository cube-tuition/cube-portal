...
function cartesianSvg(b) {
  ...
  const xDot = (xi) => { s += `<circle cx="${X(xi)}" cy="${Y(0)}" r="3.4" fill="#0F766E"/><text x="${X(xi) + 7}" y="${Y(0) + 16}" font-size="11" font-weight="bold" fill="#0F766E">(${fmtN(xi)}, 0)</text>` }
  const yDot = (yi) => { s += `<circle cx="${X(0)}" cy="${Y(yi)}" r="3.4" fill="#0F766E"/><text x="${X(0) + 7}" y="${Y(yi) - 6}" font-size="11" font-weight="bold" fill="#0F766E">(0, ${fmtN(yi)})</text>` }
  ...
  for (const cv of parseCurveRows(b.lines)) {
    ...
    if (cv.label && labelPt) s += `<text x="${X(labelPt[0]) + 6}" y="${Y(labelPt[1]) - 6}" font-size="11.5" font-weight="bold" fill="#325099">${esc(cv.label)}</text>`
    if (showNums && paths) {
