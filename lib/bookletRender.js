...
export function blockHtml(b, ctx) {
  switch (b.type) {
    ...
    case 'image': {
      // Standalone centred image with an optional caption and width (% of page).
      if (!b.image) return ''
      ...
      return `<div class="bk-block bk-img bk-img-center bk-img-block"><img src="${esc(url || '')}" alt=""${style} />${cap}</div>`
    }
...
export function bookletRenderItems(blocks, { solutions, meta } = {}) {
  ...
  // their homework questions don't show marks.
  const isMaths = /maths/i.test(meta?.subject || '')
