...
export async function loadExam(id) {
  ...
  return {
    ...
    sections: (sections || []).map((s) => ({
      _key: s.id, type: s.type, marks_limit: s.marks_limit, allow_time: s.allow_time, note: s.note,
 ...
 *
 * Any section will take it, whatever its type. A section's type decides the
 * instructions under its heading, not what may sit in it: the renderer prints
 * each question by its own qtype, so a multiple-choice question carried into an
 * extended section still prints its options and one carried the other way still
 * prints working lines.
 */
const neighbour = (sections, si, dir) => {
  const j = si + dir
  return j >= 0 && j < sections.length ? j : -1
}
 ...
 * three sections away.
 *
 * Any section accepts it — see `neighbour`. Returns the same array back when
 * the move cannot be made at all (an unknown section or slot), which leaves the
 * drop a no-op.
 */
...
export function buildExamRenderPayload({ exam, questions = [], rubrics = [] }) {
  ...
  const sections = (exam?.sections || []).map((s, i) => ({
    roman: EXAM_ROMAN[i] || String(i + 1), type: s.type, allow: s.allow_time, note: s.note || '',
    questions: (s.slots || [])
      .map((sl) => {
