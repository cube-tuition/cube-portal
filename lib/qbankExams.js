...
export const blankSlot = () => ({ _key: key(), topic_id: null, subtopic_id: null, skill_id: null, difficulty: null, question_id: null, working_lines: null, page_breaks: null, rubric_id: null, custom_rubric: null, show_notes: true, notes: '' })
...
export function examTitle({ yearLabel, paperType, termYear2, term, kind }) {
  const s = paperType === 'english' ? 'E' : 'M'
  return `${yearLabel || '?'}.${s}. ${termYear2 || '2X'}T${term ?? 'X'} ${kind === 'mid_term' ? 'MTT' : 'TT'}`
}
...
export const isAutoExamTitle = (t) =>
  !t || /^Untitled/i.test(t) || /^[\d/?]+\.[ME]\. (\d{2}|2X)T[\dX] (TT|MTT)$/.test(String(t).trim())
...
export async function createExam(createdBy, paperType = 'maths', term = null, kind = 'term') {
  const { data: exam, error } = await supabase.from(T_QBANK_EXAMS)
    .insert({
      title: kind === 'mid_term' ? 'Untitled mid-term test' : 'Untitled term test',
      created_by: createdBy || null,
      paper_type: paperType === 'english' ? 'english' : 'maths',
      term: term ?? null, kind: kind === 'mid_term' ? 'mid_term' : 'term',
    }).select('*').single()
