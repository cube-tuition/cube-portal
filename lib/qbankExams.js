...
export async function createExam(createdBy, paperType = 'maths', term = null) {
  const { data: exam, error } = await supabase.from(T_QBANK_EXAMS)
    .insert({ title: 'Untitled exam', created_by: createdBy || null, paper_type: paperType === 'english' ? 'english' : 'maths', term: term ?? null }).select('*').single()
