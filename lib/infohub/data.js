...
export const PAGE_COLS =
  'id, slug, title, icon, summary, category_id, parent_id, draft, published, status, tags, visible_roles, pinned, mandatory, sort_order, scheduled_at, updated_at, updated_by_name, published_at'
...
export async function listNavPages() {
  const { data } = await supabase.from('infohub_pages')
    .select('id, slug, title, icon, category_id, parent_id, pinned, mandatory, sort_order, visible_roles')
    .eq('status', 'published')
    .order('sort_order')
  return data || []
}
...
// Just enough of a page to link back to it.
export async function loadPageCrumb(id) {
  if (!id) return null
  const { data } = await supabase.from('infohub_pages').select('slug, title, icon').eq('id', id).maybeSingle()
  return data || null
}
...
// ── Pages: mutations ──────────────────────────────────────────────────────────
export async function createPage({ title = 'Untitled page', blocks = [], category_id = null, icon = null, parent_id = null } = {}) {
  const slug = await uniqueSlug(slugify(title))
  const { data } = await supabase.from('infohub_pages')
    .insert({ title, slug, draft: blocks, status: 'draft', category_id, icon, parent_id })
    .select(PAGE_COLS).single()
  return data
}

/*
 * Save an edit made on the page itself (see components/infohub/InlineBlocks).
 *
 * A published page is edited in place: the reader is looking at the live copy,
 * so the live copy is what changes, and draft stays level with it. A page that
 * is still a draft only moves its draft — editing one in place must not publish
 * it by the back door.
 */
export async function saveInline(page, blocks, { editorName = '' } = {}) {
  if (!page?.id) throw new Error('No page to save')
  const now = new Date().toISOString()
  const patch = { draft: blocks, updated_at: now }
  if (editorName) patch.updated_by_name = editorName
  if (page.status === 'published') { patch.published = blocks; patch.published_at = now }
  const { error } = await supabase.from('infohub_pages').update(patch).eq('id', page.id)
  if (error) throw error
  return patch
}

// A page's subpages, in sidebar order. Published only for viewers; the read
// policy hides a Director parent's children from teachers along with the parent.
export async function listSubpages(parentId, { publishedOnly = true } = {}) {
  if (!parentId) return []
  let q = supabase.from('infohub_pages')
    .select('id, slug, title, icon, summary, status, mandatory, sort_order')
