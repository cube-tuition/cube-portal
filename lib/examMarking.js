...
export async function loadExamItems(examId) {
  ...
  if (qids.length) {
    ...
    const { data: qs } = await supabase.from('qbank_questions')
      .select('id, qtype, marks, is_multipart, stem_latex, qbank_question_parts(marks), qbank_topics(name), qbank_subtopics(qbank_topics(name)), qbank_skills(qbank_topics(name))')
      .in('id', qids)
