...
export async function resolveAssignedExamId(classId, termNumber, kind = 'term') {
  ...
  if (linkedAll.length) {
    ...
    const kindOf = Object.fromEntries((kinds || []).map((e) => [e.id, e.kind === 'mid_term' ? 'mid_term' : 'term']))
  ...
  // The name-based fallback predates mid-terms; it only ever finds term tests.
...
export async function loadExamItems(examId) {
  ...
  if (qids.length) {
    ...
    const { data: qs, error: qErr } = await supabase.from('qbank_questions')
      .select('id, qtype, marks, is_multipart, stem_latex, qbank_question_parts(marks), qbank_topics(name), qbank_subtopics!qbank_questions_subtopic_id_fkey(qbank_topics(name)), qbank_skills!qbank_questions_skill_id_fkey(qbank_topics(name))')
      .in('id', qids)
  ...
  ;(ex?.sections || []).forEach((s, si) => {
    ...
    ;(s.slots || []).forEach((sl) => {
      ...
      if (!q) return
      const topicName = q.qbank_subtopics?.qbank_topics?.name
