...
import { questionTotalMarks as qMax } from './qbank'
 ...
 *
 * Topic, plus its subtopic where that says something: "Measurement · Area"
 * rather than a single Measurement row covering length, area, volume and mass.
 * A subtopic named "General" is the bank's placeholder for "no breakdown here"
 * (51 topics use one), so it reports as the bare topic — as does a question
 * with no subtopic at all.
 */
const GENERIC_SUBTOPIC = /^(general|other|misc(ellaneous)?)$/i
...
export function analysisTopicLabel(q) {
  ...
  return (name && !GENERIC_SUBTOPIC.test(name) && name !== topic) ? `${topic} · ${name}` : topic
}
...
export async function loadExamItems(examId) {
  ...
  if (qids.length) {
    ...
    const { data: qs, error: qErr } = await supabase.from('qbank_questions')
      .select('id, qtype, marks, is_multipart, stem_latex, qbank_question_parts(marks), qbank_topics(name), qbank_subtopics!qbank_questions_subtopic_id_fkey(name, qbank_topics(name)), qbank_skills!qbank_questions_skill_id_fkey(qbank_topics(name))')
      .in('id', qids)
  ...
  ;(ex?.sections || []).forEach((s, si) => {
    ...
    ;(s.slots || []).forEach((sl) => {
      ...
      if (!q) return
      const topicName = analysisTopicLabel(q)
      const criteria = q.qtype === 'mcq' ? null : criteriaOf(sl)
      const critSum = criteria ? criteria.reduce((a, c) => a + c.max, 0) : 0
      items.push({
        qid: q.id, n: items.length + 1, section: label,
        topic: topicName, max: (criteria && critSum) ? critSum : qMax(q),
        qtype: q.qtype, stem: (q.stem_latex || '').replace(/\$/g, '').replace(/\s+/g, ' ').trim().slice(0, 70),
        criteria,
      })
    })
  })
