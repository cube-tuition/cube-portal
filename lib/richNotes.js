...
export function sanitizeHtml(html) {
  ...
  const clean = (node) => {
    for (const child of [...node.childNodes]) {
      ...
      // The one attribute that survives: a width/height in px on table parts,
      // which is how a resized column or row is remembered.
      for (const a of [...child.attributes]) {
        if (a.name === 'style' && /^(TABLE|TD|TH|TR)$/.test(tag)) {
          const kept = a.value.split(';').map(d => d.trim()).filter(d => /^(width|height)\s*:\s*\d+(\.\d+)?px$/i.test(d))
