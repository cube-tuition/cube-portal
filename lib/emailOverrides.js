...
/*
 * Per-report exclusions. A report is uniquely a (term, student, class, kind);
 * an excluded report is not attached when emailing the family. Absence of a row
 * = included (the default). The kind keeps a mid-term exclusion from also
 * dropping the student from the end-of-term send.
 */
...
export async function loadReportExclusions(termId, kind = 'end_of_term') {
...
export async function setReportExcluded(termId, studentId, classId, excluded, updatedBy, kind = 'end_of_term') {
  if (excluded) {
    return supabase.from(T_EMAIL_REPORT_EXCLUSIONS).upsert({
      term_id: termId, student_id: studentId, class_id: classId, kind,
      updated_by: updatedBy || null,
    }, { onConflict: 'term_id,student_id,class_id,kind' })
  }
  return supabase.from(T_EMAIL_REPORT_EXCLUSIONS)
    .delete()
    .eq('term_id', termId).eq('student_id', studentId).eq('class_id', classId).eq('kind', kind)
}
