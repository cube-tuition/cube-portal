 ...
 * "3 weeks ago", "on 3 Jan 2025".
 *
 * For "when did this last happen" lines, where the SHAPE of the gap is what
 * the reader is judging — a month-old availability needs chasing, and "on 14
 * Aug" makes you do that arithmetic yourself. Past about two months the date
 * itself is the more useful answer, so it says that instead.
 */
export const fmtSince = iso => {
  if (!iso) return null
...
/*
 * The code is ALWAYS composed for display and never written into a stored name.
 * The builder header, the Master Database and the Materials pages each prepend
 * it themselves, so a title saved as "9.M. Algebra 1" would render as
 * "9.M. 9.M. Algebra 1". Store the bare name; label it on the way out.
 */
const coded = (year, subject, base) =>
  (year && subject) ? `${year}.${subjectCode(subject)}. ${base}` : base

/**
 * "9.M. Linear Relationships 3" for a row from `booklets`.
 * Chemistry names stored as "M3W2" display as "M3L2" (Chemistry counts lessons).
 */
export const bookletLabel = (b) =>
  coded(b?.year, b?.subject, fmtWorkbookCode(b?.booklet_name, b?.subject) ?? '')

/** The same label for a row from `booklet_builds`, whose name column is `title`. */
export const buildLabel = (b, fallback = 'Untitled workbook') =>
  coded(b?.year, b?.subject, (fmtWorkbookCode(b?.title, b?.subject) || '').trim() || fallback)
