 ...
 *   type       'text'|'singleSelect'|'linkedRecord'|'email'|'phone'|'integer'|
 *              'number'|'date'|'time'|'boolean'|'json'
...
export const TABLE_META = {
  ...
  courses: {
    ...
    columns: {
      ...
      course_price: { label: 'Price', type: 'number' },
      created_at:   { label: 'Created', type: 'date', readOnly: true, hidden: true },
    },
  },
  classes: {
    ...
    columns: {
      ...
      teacher:     { label: 'Teacher (text)', deprecated: 'Free-text name; does not link to the Tutors table. Future: tutor_id foreign key.', help: 'Display name only — not linked to a tutor record.' },
      room:        { label: 'Room' },
  ...
  enrolments: {
    ...
    columns: {
      ...
      price:            { label: 'Price', type: 'number', help: 'Per-term price. Editing asks for confirmation.' },
  ...
  invoices: {
    ...
    columns: {
      ...
      subtotal:              { label: 'Subtotal', type: 'number', readOnly: true, help: 'Generated — edit line items via the invoice tools, not here.' },
      sibling_discount:      { label: 'Sibling Disc.', type: 'number', readOnly: true },
      multi_course_discount: { label: 'Multi-course Disc.', type: 'number', readOnly: true },
      total:                 { label: 'Total', type: 'number', readOnly: true },
  ...
  shifts: {
    ...
    columns: {
      ...
      rate_snapshot: { label: 'Rate', type: 'number', readOnly: true },
...
export function validateValue(table, col, value) {
  ...
  if (value !== null && String(value) !== String(value).trim()) {
    return `Value has leading/trailing whitespace (“${String(value)}”).`
  }
  switch (m.type) {
