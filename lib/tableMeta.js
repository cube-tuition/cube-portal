...
const YEAR_OPTIONS   = ['K','1','2','3','5','6','7','8','9','10','11','12']   // no Year 4 — CUBE doesn't teach it
...
export const TABLE_META = {
  students: {
    ...
    columns: {
      ...
      status:    { label: 'Status', type: 'singleSelect', options: ['active','pending','trial','inactive'], required: true, help: 'active = enrolled & attending · pending = prospective lead (not yet enrolled) · trial = on a trial · inactive = left / disenrolled.' },
      payment_method: { label: 'Payment', type: 'singleSelect', options: ['bank','cash'], hidden: true, help: 'How this family pays — edited per family in the Guardians view (kept on students as the underlying storage). New invoices inherit it; a family invoice is cash if any member is cash.' },
      notes:     { label: 'Notes', type: 'longtext', hidden: true, help: 'Free-text notes about this student. Edited in the student directory detail panel (not shown as a grid column).' },
  ...
  guardians: {
    ...
    columns: {
      ...
      // Synthetic columns for the grouped data view (one row per guardian person)
      students:       { label: 'Students', readOnly: true, help: 'All students linked to this guardian — the data view groups one row per guardian.' },
      payment_method: { label: 'Payment', type: 'singleSelect', options: ['bank','cash'], help: 'How this family pays — writes to every linked student, same as the Families view toggle. New invoices inherit it.' },
    },
  },
  tutors: {
    label: 'Tutors',
  ...
  courses: {
    label: 'Courses',
    columns: {
      // The course CODE is how staff identify a course ("8.MC"), so it carries
      // the "Course ID" label. The integer id stays the real primary key that
      // classes.course_id points at — it is just hidden, since two competing
      // "IDs" on screen is what caused the confusion.
      id:           { label: 'Row # (internal)', readOnly: true, hidden: true },
      course_name:  { label: 'Course Name', required: true },
      course_code:  { label: 'Course ID', required: true, help: 'Unique short code that identifies the course, e.g. 8.MC.' },
      ...
      active:       { label: 'Active', type: 'boolean', help: 'Set Inactive to retire a course — history (classes, enrolments) is kept. New courses start Active.' },
      created_at:   { label: 'Created', type: 'date', readOnly: true, hidden: true },
    },
  },
  classes: {
    ...
    columns: {
      ...
      room:        { label: 'Room' },
      status:      { label: 'Status', type: 'singleSelect', options: ['active','inactive'], required: true, help: 'inactive = class no longer runs (e.g. its 1:1 student left) — hidden from the calendar and skipped by Generate Lessons; history is kept.' },
  ...
  enrolments: {
    ...
    columns: {
      ...
      next_term_status: { label: 'Next Term', hidden: true, readOnly: true, help: 'Legacy — the per-enrolment continuation tracking was retired; every enrolment rolls over at term transition and non-continuing students are disenrolled manually in the new term.' },
...
// ── Value normalisation (opt-in, applied by editors before validate/save) ─────

/*
 * Forgiving 24h time parser. People type times all sorts of ways — "9:30",
 * "9.30", "0930", "4pm", "4:15 pm", "16", "18:00:00" — and every one of those
 * has exactly one sensible meaning. Returns canonical "HH:MM", or null when the
 * input isn't confidently a time (in which case callers keep the raw string and
 * let validation warn as before — normalisation never invents data).
 */
export function normalizeTime(value) {
  const s = String(value ?? '').trim().toLowerCase()
  if (s === '') return null
  const m = s.match(/^(\d{1,2})(?:[:.]?([0-5]\d))?(?::([0-5]\d))?\s*(am|pm)?$/)
  if (!m) return null
  let h = Number(m[1])
  const min = m[2] != null ? Number(m[2]) : 0
  const ap = m[4]
  // "930" parses as h=93 via the lazy branch — re-split bare 3-4 digit strings
  if (m[2] == null && /^\d{3,4}$/.test(m[1])) {
    const digits = m[1].padStart(4, '0')
    h = Number(digits.slice(0, 2))
    return normalizeTime(`${h}:${digits.slice(2)}${ap ?? ''}`)
  }
  if (ap === 'pm' && h < 12) h += 12
  if (ap === 'am' && h === 12) h = 0
...
/**
 * Canonical form of a typed value for its column type, or the input unchanged
 * when no (confident) normalisation applies. Currently only 'time' columns
 * normalise; everything else passes through untouched.
 */
export function normalizeValue(table, col, value) {
  const s = value === null || value === undefined ? '' : String(value).trim()
  if (s === '') return s
  if (fieldType(table, col) === 'time') return normalizeTime(s) ?? s
  return s
}
...
export function formatDisplay(table, col, value) {
  ...
  switch (fieldType(table, col)) {
    ...
    // Postgres time columns serialise as HH:MM:SS — nobody schedules seconds.
    case 'time':     return /^\d{2}:\d{2}(:\d{2})?$/.test(String(value)) ? String(value).slice(0, 5) : String(value)
