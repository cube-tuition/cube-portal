...
export async function buildPayslipPdfBase64(p) {
  ...
  y += 7
  const weeks = (p.weeks && p.weeks.length) ? p.weeks : [{ label: '', range: '', shifts: p.shifts || [], hours: p.hours, amount: p.gross }]
  for (const wk of weeks) {
    if (wk.label) {
      doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(...NAVY)
      doc.text(`${wk.label}${wk.range ? '  ·  ' + wk.range : ''}`.toUpperCase(), cDate, y, { charSpace: 0.4 })
      y += 5.5
    }
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8.5); doc.setTextColor(...INK)
    for (const s of wk.shifts) {
    ...
    if (wk.label) {
      doc.setDrawColor(...FAINT); doc.setLineWidth(0.2); doc.line(cDesc, y - 2.5, cAmt, y - 2.5)
      doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(...GREY)
      doc.text(`${wk.label} subtotal`, cDesc, y)
      doc.setTextColor(...INK)
      doc.text(Number(wk.hours || 0).toFixed(2), cHrs, y, { align: 'right' })
      doc.text(money(wk.amount), cAmt, y, { align: 'right' })
      y += 8.5
    }
  }
      ...
      <div style="font-size:15px;line-height:1.7;">
        ...
        <table style="width:100%;border-collapse:collapse;background:#F0F4FF;border:1px solid #DEE7FF;border-radius:12px;padding:0;margin:0 0 18px;">
          <tbody style="display:table;width:100%;padding:8px 16px;box-sizing:border-box;">
            ...
            ${(p.weeks && p.weeks.length > 1) ? p.weeks.map(w => row(`&nbsp;&nbsp;${w.label}${w.range ? ' (' + w.range + ')' : ''}`, money(w.amount))).join('') : ''}
