...
/*
 * The past terms ONE student should see.
 *
 * Every holiday period is a term row of its own, so a student who has been
 * here a year collects a row per school break whether or not they did a
 * holiday course — empty cards between the terms they actually attended. A
 * teaching term still shows when they were enrolled in nothing (they were a
 * CUBE student that term, and it holds their quizzes and attendance), but a
 * holiday term only earns its place if they were enrolled in a course in that
 * specific holiday period.
 *
 * `enrolledTermIds` is a Set (or any iterable) of term ids the student has an
 * enrolment in — see enrolledTermIdsFor in lib/classes.
 */
export function getPastTermsForStudent(terms, currentTermId, enrolledTermIds, now = new Date()) {
  const ids = enrolledTermIds instanceof Set ? enrolledTermIds : new Set(enrolledTermIds || [])
  return getPastTerms(terms, currentTermId, now).filter(t => !isHolidayTerm(t) || ids.has(t.id))
}
...
// Year 12 picks up that Term 4 and runs into the next calendar year, so its
// terms are listed in teaching order: Term 4, then Terms 1–3.
export const ALL_CURRICULUM_TERMS = [1, 2, 3, 4]
const Y11_CURRICULUM_TERMS = [1, 2, 3]
const Y12_CURRICULUM_TERMS = [4, 1, 2, 3]

/** The curriculum terms for a year level, in teaching order.
 *  Year 11 → [1,2,3], Year 12 → [4,1,2,3], everything else → [1,2,3,4]. */
export function curriculumTerms(year) {
  const y = Number(year)
  if (y === 11) return Y11_CURRICULUM_TERMS
  if (y === 12) return Y12_CURRICULUM_TERMS
  return ALL_CURRICULUM_TERMS
}
