...
import { T_SYLLABUS_MODULES, T_SYLLABUS_TOPICS, T_SYLLABUS_DOTPOINTS, T_BOOKLET_BUILDS } from './tables'
...
// there's no manual ticking. Returns a map: dotpointId → [booklet titles].
export async function fetchDotpointCoverage() {
  const { data } = await supabase.from(T_BOOKLET_BUILDS).select('title, syllabus_points')
  const map = {}
  for (const b of data || []) {
    const pts = Array.isArray(b.syllabus_points) ? b.syllabus_points : []
    for (const id of pts) (map[id] ||= []).push(b.title || 'Untitled booklet')
  }
  return map
}
