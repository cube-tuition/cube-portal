...
/*
 * ── Booklet dotpoint pool ────────────────────────────────────────────────────
 * A Chemistry booklet first CHOOSES its content as a checklist over the whole
 * year's syllabus (the pool, stored in booklet_builds.syllabus_points), then
 * ALLOCATES pooled dotpoints to section headers on the Content page. The
 * helpers below keep those two layers consistent. Pool ids are leaves only:
 * lone main dotpoints and subdotpoints — the same convention the section
 * picker has always stored.
 */

// The master syllabus reduced to the pooled leaves, preserving the module →
// topic → main → sub hierarchy (a pooled sub keeps its parent as the group
// header). Feeds the section picker so it can only offer pooled dotpoints.
export function filterModulesToPool(modules, poolIds) {
  const set = poolIds instanceof Set ? poolIds : new Set(poolIds || [])
  return modules
    .map((m) => ({
      ...m,
      topics: m.topics
        .map((t) => ({
          ...
          dotpoints: t.dotpoints
            .map((d) => (d.subs.length ? { ...d, subs: d.subs.filter((s) => set.has(s.id)) } : d))
            // A lone main survives if pooled itself; a main with subs survives
            // while any pooled sub remains under it (the parent prints as the
            // group header for its pooled subs).
            .filter((d) => d.subs.length > 0 || set.has(d.id)),
        }))
        .filter((t) => t.dotpoints.length),
    }))
    .filter((m) => m.topics.length)
}

// Remove dotpoints from every section block's allocation (used when they are
// unticked from the pool): filters each section's syllabus_points and
// regenerates its printed band text. Returns { blocks, touched }.
export function removeDotpointsFromSections(blocks, modules, removeIds) {
  const gone = removeIds instanceof Set ? removeIds : new Set(removeIds || [])
  let touched = 0
  const next = (blocks || []).map((b) => {
    if (b.type !== 'section' || !Array.isArray(b.syllabus_points)) return b
    const kept = b.syllabus_points.filter((id) => !gone.has(id))
    if (kept.length === b.syllabus_points.length) return b
    touched++
    return { ...b, syllabus_points: kept, syllabus: selectedToSyllabusText(modules, kept) }
  })
  return { blocks: next, touched }
}

// leafId → [{ id, number, title }] of the section blocks that draw it — the
// Content tab shows this next to each pooled dotpoint as its allocation.
export function dotpointAllocation(blocks) {
  const map = {}
  for (const b of blocks || []) {
    if (b.type !== 'section' || !Array.isArray(b.syllabus_points)) continue
    for (const id of b.syllabus_points) {
      ;(map[id] ||= []).push({ id: b.id, number: b.number, title: b.title })
    }
  }
  return map
}
...
// Module number → module name, across every year of a subject: Chemistry
// numbers its modules 1–4 in Year 11 and 5–8 in Year 12, so the numbers never
// collide and one map labels both years. Rows read "Module 3: Reactive
// Chemistry"; the leading number is what a booklet name ("M3L2") carries.
//
// The list is small, static reference data read by every screen that lists
// Chemistry booklets, so the fetch is cached per subject for the session.
const moduleNameCache = {}
export function fetchModuleNames(subject) {
  return (moduleNameCache[subject] ||= loadModuleNames(subject))
}
async function loadModuleNames(subject) {
  const { data } = await supabase.from(T_SYLLABUS_MODULES)
    .select('name, sort_order').eq('subject', subject).order('year').order('sort_order')
  const out = {}
  for (const m of data || []) {
    const n = /^\s*Module\s+(\d+)/i.exec(m.name || '')
    if (n) out[Number(n[1])] = m.name
  }
  return out
}
