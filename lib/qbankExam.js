...
/*
 * The second line of the cover: the paper's own title ("11.C. 26T3 MOCK"), which
 * is what the paper is called everywhere else — in the exam list, in the
 * filename, and on the timetable. It used to read a generic label built from the
 * kind, so every mock announced itself as "Mock Examination" and two different
 * papers were indistinguishable on their covers. An untitled exam still falls
 * back to that label rather than printing a blank line.
 */
function coverTitleLine(meta) {
  const title = String(meta.title ?? '').trim()
  return title ? esc(title) : coverKindLabel(meta)
}
