...
function solutionBlocks(q, number) {
  ...
  if (parts.length) {
    parts.forEach((p, i) => {
      const lbl = p.part_label || 'abcdefgh'[i] || String(i + 1)
      out.push(criteriaTable(`(${esc(lbl)})`, p.marks ?? 1, p.criteria, latexToHtml(p.solution_latex || '')))
    })
  } else {
    out.push(criteriaTable('', questionMarks(q) || 1, q.criteria, latexToHtml(q.solution_latex || '')))
  }
  return out
}
