...
function extendedPaperBlocks(q, number) {
  ...
  head.innerHTML =
    `<div style="margin-bottom:4px">
     ...
     <div style="font-size:15px;line-height:1.5">${richToHtml(q.stem_latex || '')}</div>${imagesHtml(stemImagesOf(q))}`
  ...
  if (parts.length) {
    ...
    tail.innerHTML = isSketchText(q.stem_latex) ? sketchSpace() : dottedLines(qlines)
...
function solutionBlocks(q, number, paperType) {
  ...
  if (stemImgs.length) {
    const imgEl = document.createElement('div')
    imgEl.innerHTML = imagesHtml(stemImgs)
