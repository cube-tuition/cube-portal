...
// A blank, transparent Cartesian plane for "sketch" questions — gridlines,
// axes with arrowheads, numbered units and x / y labels.
function cartesianPlane(size = 400, units = 5) {
  const m = 18                              // margin so edge labels/arrows aren't clipped
  const step = (size - 2 * m) / (2 * units)
  const c = size / 2
  const pos = (i) => c + i * step
  let svg = ''
  // gridlines (inset within the margin)
  for (let i = -units; i <= units; i++) {
    if (i === 0) continue
    const p = pos(i)
    svg += `<line x1="${p}" y1="${m}" x2="${p}" y2="${size - m}" stroke="#d6d6d6" stroke-width="0.7"/>`
    svg += `<line x1="${m}" y1="${p}" x2="${size - m}" y2="${p}" stroke="#d6d6d6" stroke-width="0.7"/>`
  }
  // axes extend a little past the grid to carry arrowheads
  const lo = m - 8, hi = size - m + 8
  svg += `<line x1="${lo}" y1="${c}" x2="${hi}" y2="${c}" stroke="#1a1a1a" stroke-width="1.4"/>`
  svg += `<line x1="${c}" y1="${lo}" x2="${c}" y2="${hi}" stroke="#1a1a1a" stroke-width="1.4"/>`
  // unit numbers (y-axis values increase upward → screen pos(-i))
  for (let i = -units; i <= units; i++) {
    if (i === 0) continue
    svg += `<text x="${pos(i)}" y="${c + 13}" text-anchor="middle" font-family="Georgia, serif" font-size="9" fill="#666">${i}</text>`
    svg += `<text x="${c - 6}" y="${pos(-i) + 3}" text-anchor="end" font-family="Georgia, serif" font-size="9" fill="#666">${i}</text>`
  }
  // arrowheads
  svg += `<path d="M ${hi} ${c} L ${hi - 9} ${c - 4} L ${hi - 9} ${c + 4} Z" fill="#1a1a1a"/>`
  svg += `<path d="M ${lo} ${c} L ${lo + 9} ${c - 4} L ${lo + 9} ${c + 4} Z" fill="#1a1a1a"/>`
  svg += `<path d="M ${c} ${lo} L ${c - 4} ${lo + 9} L ${c + 4} ${lo + 9} Z" fill="#1a1a1a"/>`
  svg += `<path d="M ${c} ${hi} L ${c - 4} ${hi - 9} L ${c + 4} ${hi - 9} Z" fill="#1a1a1a"/>`
  // axis letters
  svg += `<text x="${hi - 2}" y="${c - 8}" font-family="Georgia, serif" font-style="italic" font-size="14" fill="#1a1a1a">x</text>`
  svg += `<text x="${c + 9}" y="${lo + 5}" font-family="Georgia, serif" font-style="italic" font-size="14" fill="#1a1a1a">y</text>`
  return `<div style="display:flex;justify-content:center;margin:12px 0 6px"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">${svg}</svg></div>`
}
...
function extendedPaperBlock(q, number) {
  ...
  if (parts.length) {
    parts.forEach((p, i) => {
      ...
      body += isSketchText(p.prompt_latex) ? cartesianPlane() : dottedLines(plines)
    })
  } else {
    const qlines = (wl && wl['_'] != null) ? Math.max(0, wl['_']) : Math.max(3, marks * 2)
    body += isSketchText(q.stem_latex) ? cartesianPlane() : dottedLines(qlines)
  }
  const el = document.createElement('div')
...
function solutionBlocks(q, number) {
  ...
  // Build the answer boxes for this question (one per part, or one overall).
  const tables = parts.length
    ? parts.map((p, i) => {
        const lbl = p.part_label || 'abcdefgh'[i] || String(i + 1)
  ...
  group.appendChild(tables.shift())
...
// ── Main ──────────────────────────────────────────────────────────────────────
export async function exportExamPdf({ meta, sections, solutions = false, preview = false }) {
