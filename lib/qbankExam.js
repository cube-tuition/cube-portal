...
function extendedPaperBlocks(q, number) {
  ...
  head.innerHTML =
    `<div style="margin-bottom:4px">
     ...
     <div style="font-size:15px;line-height:1.5">${latexToHtml(q.stem_latex || '')}</div>${imagesHtml(stemImagesOf(q))}`
  ...
  if (parts.length) {
    ...
    parts.forEach((p, i) => {
      ...
      let body = `<div style="margin:20px 0 16px;font-size:15px;line-height:1.5">${pm}<span style="font-weight:600">${esc(lbl)}.</span>&nbsp;${latexToHtml(p.prompt_latex || '')}</div>`
  ...
  if (q._rubric) {
    ...
    if (parts.length) {
      tables = parts.map((p, i) => {
        ...
        const box = document.createElement('div')
        box.style.cssText = `border:1px solid ${INK};padding:8px 10px;margin:14px 0 4px;break-inside:avoid`
        const promptHtml = p.prompt_latex ? `<div style="font-size:13px;color:#555;margin-bottom:4px">${latexToHtml(p.prompt_latex)}</div>` : ''
        const solHtml = latexToHtml(p.solution_latex || '') || '<span style="color:#999">—</span>'
