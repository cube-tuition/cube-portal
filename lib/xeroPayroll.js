...
export async function createDraftPayRun(payrollCalendarId) {
  // Payroll AU POST bodies are BARE ARRAYS (the API's UpdatePayRunRequest is a
  // list type) — wrapping in { PayRuns: [...] } 400s with a deserialisation error.
  const json = await payrollFetch('/PayRuns', {
    method: 'POST',
    body: [{ PayrollCalendarID: payrollCalendarId }],
  })
...
/**
 * Set a DRAFT pay run's payment date (yyyy-mm-dd). The calendar's default can
 * be nonsense for term-aligned payrolls (e.g. a payment date inside the
 * fortnight); the push corrects it to the real pay day. Draft runs only.
 */
export async function setPayRunPaymentDate(payRunID, paymentDate) {
  return await payrollFetch(`/PayRuns/${payRunID}`, {
    method: 'POST',
    body: [{ PayRunID: payRunID, PaymentDate: paymentDate }],
  })
}

/**
 * Set the ordinary-hours earnings lines on a DRAFT payslip.
 *
 * ONE LINE PER RATE. A teacher's fortnight routinely spans several $/h —
 * resolve_matrix_rate prices each class separately, so e.g. 8h @ $50 + 6h @ $60
 * is normal. Collapsing that into one line would pay all 14 hours at whichever
 * rate won, which is a mispay; Xero AU accepts repeated EarningsRateIDs so long
 * as each line carries its own RatePerUnit.
 *
 * The portal's rate is always sent: it's what the payroll page totals and what
 * the payslip PDFs show, and it's the only figure that can be right for a
 * mixed-rate fortnight. Xero's pay-template rate applies only to a line whose
 * rate is null (an unpriced shift).
 *
 * lines: [{ hours, rate }] — rate null ⇒ let Xero's template rate apply.
 * Only ever call this for payslips on a DRAFT pay run.
 */
export async function setPayslipHours(payslipId, { earningsRateId, lines }) {
  const earningsLines = (lines || [])
    .filter(l => Number(l.hours) > 0)
    .map(l => {
      const line = { EarningsRateID: earningsRateId, NumberOfUnits: Number(l.hours) }
      if (l.rate != null && Number.isFinite(Number(l.rate))) line.RatePerUnit = Number(l.rate)
      return line
    })
  if (!earningsLines.length) throw new Error('no hours to write onto the payslip')
  // Same bare-array rule as PayRuns: the body is an array of PayslipLines objects.
  return await payrollFetch(`/Payslip/${payslipId}`, {
    method: 'POST',
    body: [{ EarningsLines: earningsLines }],
  })
}
