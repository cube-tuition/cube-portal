...
export async function loadLevelTestItems(blocks = []) {
  ...
  if (bankIds.length) {
    ...
    const { data: qs, error: qErr } = await supabase.from('qbank_questions')
      .select('id, qtype, marks, is_multipart, qbank_question_parts(marks), qbank_topics(name), qbank_subtopics!qbank_questions_subtopic_id_fkey(qbank_topics(name)), qbank_skills!qbank_questions_skill_id_fkey(qbank_topics(name))')
      .in('id', bankIds)
