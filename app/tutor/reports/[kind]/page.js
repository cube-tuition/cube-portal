...
/*
 * Class picker for one kind of report — /tutor/reports/[kind]
 *
 * Pick a class + term → open /tutor/reports/[kind]/[classId]/[termId], which
 * renders a printable page with one report per enrolled student. Admin uses the
 * browser's "Save as PDF" to bundle them into one file.
 *
 * The kind (mid-term / end-of-term) comes from the URL and decides the copy,
 * which weeks the charts cover, and which comment rows the progress count reads
 * — mid-term and end-of-term comments are separate rows.
 */
      ...
      <section className="bg-gradient-to-r from-[#F8FAFF] via-[#EEF4FF] to-[#BFD1FF] border-b border-[#DEE7FF]">
        <div className="max-w-7xl mx-auto px-6 md:px-10 py-10 md:py-12">
          ...
          <p className="text-sm md:text-base text-[#2A2035]/70 mt-2 max-w-2xl">
            Pick a class to open its bundle. {kind.blurb}
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {filtered.map(c => {
              ...
              return (
                <Link
                  key={c.id}
                  href={`/tutor/reports/${kind.slug}/${c.id}/${termId}`}
