...
export async function POST(request) {
  try {
    ...
    const { subject, body, highlight, cta, families, test, testEmail } = await request.json()
    ...
    if (test && !families?.length) {
      ...
      const { error } = await resend.emails.send({
        ...
        html: buildCourseOfferEmailHtml(body, { parentName: 'there', studentNames: 'your child' }, highlight, cta),
      })
    ...
    for (const family of families) {
      ...
      const { error: sendErr } = await resend.emails.send(applyEmailTestMode({
        ...
        bcc:  [PORTAL_BCC],
        subject,
        html: buildCourseOfferEmailHtml(body, { parentName: family.parent_name, studentNames: family.student_names }, highlight, cta),
      }, test))
      results.push({
