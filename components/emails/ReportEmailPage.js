 ...
 * /tutor/emails/mid-term. Everything that differs between them (page copy,
 * default template and subject, storage folder, and which rows count as
 * "sent" / "excluded") comes from the kind, so the two can never drift.
 *
 * Workflow:
 *   1. Select a term
 *   2. Upload a PDF report per student (Supabase Storage — path per kind)
 *   3. Preview family groupings — siblings combined into one email
 *   4. Send via Resend API
 *
 * Prerequisites:
 *   - A "term-reports" bucket in Supabase Storage (Dashboard → Storage)
 *   - RESEND_API_KEY + SUPABASE_SERVICE_ROLE_KEY in .env.local
 */

const STORAGE_BUCKET = REPORT_BUCKET
...
const resultsKey = (termId, kindKey) =>
  kindKey === 'end_of_term' ? `eot_results_${termId}`   // unchanged: keeps every existing record
                            : `${kindKey}_results_${termId}`
...
async function saveResultsToDb(termId, kindKey, results) {
  if (!termId) return
  try {
    const { data } = await supabase.from('portal_settings').select('value').eq('key', resultsKey(termId, kindKey)).maybeSingle()
    let existing = []
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function ReportEmailPage({ kind: kindKey = 'end_of_term' }) {
  const router = useRouter()
  const kind = kindByKey(kindKey)
  const DEFAULT_TEMPLATE = kind.defaultTemplate
  ...
  const SUBJECT_KEY  = kindKey === 'end_of_term' ? 'cube_eot_subject'  : `cube_${kindKey}_subject`
  const DEFAULT_SUBJECT = kind.defaultSubject
  ...
  const localKey = termId ? `cube_${resultsKey(termId, kindKey)}` : null
  const [results,  setResults]  = useState(() => {
    if (typeof window !== 'undefined' && termId) {
      try { return JSON.parse(localStorage.getItem(`cube_${resultsKey(termId, kindKey)}`)) || [] } catch { return [] }
    }
    return []
  })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      let local = []
      try { local = JSON.parse(localStorage.getItem(`cube_${resultsKey(termId, kindKey)}`) || '[]') || [] } catch {}
      if (cancelled) return
      const merged = mergeResults(db, local)
      setResults(merged)
      loadedRef.current = true
      // One-time migration: push any browser-only sends up to the shared store.
      if (merged.length > db.length) saveResultsToDb(termId, kindKey, merged)
    })()
  ...
  useEffect(() => {
    ...
    if (termId && loadedRef.current) saveResultsToDb(termId, kindKey, results)
  }, [results, localKey, termId, kindKey])
  ...
  useEffect(() => {
    loadEmailOverrides(termId, kindKey).then(setOverrides)
    loadReportExclusions(termId, kindKey).then(setExcludedReports)
  }, [termId])
  ...
  const toggleReport = async (s) => {
    ...
    const { error: err } = await setReportExcluded(termId, s.student_id, s.class_id, nowExcluded, profile?.full_name, kindKey)
    if (err) {
  ...
  const sendToFamilies = async (familiesToSend, test = false) => {
    ...
    const res = await authedFetch('/api/send-report-emails', {
      method:  'POST',
  ...
  const saveEditor = async () => {
    ...
    const { error: err } = await saveEmailOverride(termId, kindKey, key, editBody, profile?.full_name)
  ...
  const resetEditor = async () => {
    ...
    const key = familyKey(editFamily)
    setSavingOverride(true); setError(null)
    const { error: err } = await deleteEmailOverride(termId, kindKey, key)
    setSavingOverride(false)
    if (err) { setError(err.message); return }
